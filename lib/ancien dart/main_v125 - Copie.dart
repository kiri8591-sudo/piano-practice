...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  void _adaptRemainingWeekToReality() {
    ...
    for (final item in future) {
      ...
      // plannedDuration reste la référence du planning initial. La durée future
      // est ajustée séparément et le changement est signalé comme non lu.
  ...
  Widget _coachAdjustmentBanner(BuildContext c, List<PlanItem> items) {
    ...
    return Padding(
      ...
      child: InkWell(
        ...
        onTap: unseen
            ? () async {
                final first = items.firstWhere(_hasUnseenCoachAdjustment);
        ...
        child: Container(
          ...
          child: Row(children: [
            _coachPulseDot(c, visible: unseen),
            if (unseen) const SizedBox(width: 8),
            ...
            if (unseen) Text('Voir', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w900, color: Theme.of(c).colorScheme.primary)),
          ]),
        ),
      ),
    );
