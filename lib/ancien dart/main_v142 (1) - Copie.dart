...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  Future<T?> _showPostSessionDialog<T>({
    ...
    // Laisser Flutter terminer la transition de fermeture de la boîte précédente.
    await Future<void>.delayed(const Duration(milliseconds: 120));
    if (!mounted) return null;
    await WidgetsBinding.instance.endOfFrame;
    if (!mounted) return null;

    final context = navKey.currentState?.overlay?.context ?? navKey.currentContext;
    if (context == null) return null;

    // Décale réellement le push d'une frame : le nouveau dialogue ne partage
    // jamais la même frame de transition que le dialogue précédent.
    return Future<T?>((resolve) {
      WidgetsBinding.instance.addPostFrameCallback((_) async {
        ...
        try {
          ...
          // Une fermeture/navigation concurrente ne doit jamais laisser l'écran
          // bloqué : on termine simplement le flux de bilan.
          resolve(null);
        }
      });
    });
  ...
  Future<void> _offerDetailedProgress(Session session) async {
    ...
    await _persist();
    // Important : attendre que la boîte "Progression des étapes" soit totalement
    // retirée avant de demander l'ouverture du "Bilan express".
