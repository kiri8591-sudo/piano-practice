  ...
  Future<void> _showCoachFeedback(Session session) async {
      ...
      builder: (c) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.pop(c, 'facile'), icon: const Text('😊'), label: const Text('Facile'))),
              const SizedBox(width: 7),
              Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.pop(c, 'correct'), icon: const Text('😐'), label: const Text('Correct'))),
              const SizedBox(width: 7),
              Expanded(child: OutlinedButton.icon(onPressed: () => Navigator.pop(c, 'difficile'), icon: const Text('😓'), label: const Text('Difficile'))),
            ]),
          ],
        ),
      ),
    ...
    await showDialog<void>(
      ...
      builder: (c) => AlertDialog(
        ...
        actions: [FilledButton(onPressed: () => Navigator.pop(c), child: const Text('C’est parti'))],
      ),
    );
  }
...
class CoachHome extends StatelessWidget {
  const CoachHome({
    ...
    required this.badges, required this.onOpenBadges, required this.onOpenBilan,
  ...
  final VoidCallback onOpenBadges; final VoidCallback onOpenBilan;
  ...
  PlanItem? _bestPending(List<PlanItem> items, DateTime now) {
    ...
    final ranked = [...items];
    ranked.sort((a, b) {
      final sa = _coachScore(a, projectById(a.projectId), now);
      final sb = _coachScore(b, projectById(b.projectId), now);
  ...
  Widget build(BuildContext c) {
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(20, 16, 20, 28), children: [
      ...
      const SizedBox(height: 18),
      Row(children: [Expanded(child: _action('Bilan', Icons.insights, onOpenBilan)), const SizedBox(width: 8), Expanded(child: _action('Planning', Icons.calendar_month, onQuickPlan)), const SizedBox(width: 8), Expanded(child: _action('Badges $badgeCount/${badges.length}', Icons.emoji_events, onOpenBadges))]),
        ...
        FilledButton(
          onPressed: () => Navigator.pop(
            ...
            Session(
              ...
              coachFocus: widget.existing?.coachFocus,
              coachRecommendation: widget.existing?.coachRecommendation,
            ),
          ),
...
class _ProjectEvolutionSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final hasTempo = ordered.any((s) => (s.newTempo ?? s.previousTempo ?? 0) > 0) || project.currentTempo > 0;
    final feelings = ordered.where((s) => s.coachFeeling != null).toList();
    return Card(margin: EdgeInsets.zero, child: Padding(padding: const EdgeInsets.fromLTRB(12, 11, 12, 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Icon(Icons.show_chart, size: 18, color: Theme.of(context).colorScheme.primary), const SizedBox(width: 7), const Expanded(child: Text('Évolution du morceau', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 13))), Text('${ordered.length} séances', style: const TextStyle(fontSize: 11, color: Colors.grey))]),
      const SizedBox(height: 10),
      if (hasStageData) ...[const Text('4 étapes', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700)), const SizedBox(height: 4), SizedBox(height: 150, child: _StageEvolutionChart(sessions: ordered, project: project)), const SizedBox(height: 4), Wrap(spacing: 10, runSpacing: 4, children: const [_LegendDot(label: 'Lecture'), _LegendDot(label: 'Mains ensemble'), _LegendDot(label: 'Mémorisation'), _LegendDot(label: 'Interprétation')])],
      if (hasTempo) ...[const SizedBox(height: 12), Row(children: [const Text('Tempo', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700)), const Spacer(), if (project.targetTempo > 0) Text('cible ${project.targetTempo} BPM', style: const TextStyle(fontSize: 10, color: Colors.grey))]), const SizedBox(height: 4), SizedBox(height: 105, child: _TempoEvolutionChart(sessions: ordered, project: project, targetTempo: project.targetTempo))],
      if (feelings.isNotEmpty) ...[const SizedBox(height: 10), const Text('Ressenti séance après séance', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700)), const SizedBox(height: 6), SizedBox(height: 34, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: feelings.length, separatorBuilder: (_, __) => const SizedBox(width: 8), itemBuilder: (_, i) { final s=feelings[i]; final emoji=s.coachFeeling=='facile'?'😊':s.coachFeeling=='difficile'?'😓':'😐'; return Tooltip(message:'${s.date.day}/${s.date.month}/${s.date.year}', child: Container(width:34,height:34,alignment:Alignment.center,decoration:BoxDecoration(border:Border.all(color:Theme.of(context).dividerColor),borderRadius:BorderRadius.circular(10)),child:Text(emoji,style:const TextStyle(fontSize:19)))); }))],
    ])));
  }
}

class _LegendDot extends StatelessWidget { const _LegendDot({required this.label}); final String label; @override Widget build(BuildContext context)=>Row(mainAxisSize:MainAxisSize.min,children:[Container(width:8,height:8,decoration:BoxDecoration(shape:BoxShape.circle,color:Theme.of(context).colorScheme.primary)),const SizedBox(width:4),Text(label,style:const TextStyle(fontSize:9,color:Colors.grey))]); }
class _StageEvolutionChart extends StatelessWidget { const _StageEvolutionChart({required this.sessions,required this.project}); final List<Session> sessions; final Project project; @override Widget build(BuildContext context)=>CustomPaint(painter:_StageEvolutionPainter(sessions,project,Theme.of(context).colorScheme),child:const SizedBox.expand()); }
class _TempoEvolutionChart extends StatelessWidget { const _TempoEvolutionChart({required this.sessions,required this.project,required this.targetTempo}); final List<Session> sessions; final Project project; final int targetTempo; @override Widget build(BuildContext context)=>CustomPaint(painter:_TempoEvolutionPainter(sessions,project,targetTempo,Theme.of(context).colorScheme),child:const SizedBox.expand()); }

class _StageEvolutionPainter extends CustomPainter {
  _StageEvolutionPainter(this.sessions,this.project,this.scheme); final List<Session> sessions; final Project project; final ColorScheme scheme;
  double? after(Session s,int i,String stage){ final direct=stage=='reading'?s.newReading:stage=='hands'?s.newHandsTogether:stage=='memory'?s.newMemory:s.newInterpretation; if(direct!=null)return direct; if(i+1<sessions.length){final n=sessions[i+1]; return stage=='reading'?n.previousReading:stage=='hands'?n.previousHandsTogether:stage=='memory'?n.previousMemory:n.previousInterpretation;} return stage=='reading'?project.reading:stage=='hands'?project.handsTogether:stage=='memory'?project.memory:project.interpretation; }
  @override void paint(Canvas canvas,Size size){const l=30.0,r=8.0,t=8.0,b=20.0;final w=size.width-l-r,h=size.height-t-b;final grid=Paint()..color=scheme.outlineVariant..strokeWidth=1;final txt=TextPainter(textDirection:TextDirection.ltr);for(final v in [0.0,.5,1.0]){final y=t+h*(1-v);canvas.drawLine(Offset(l,y),Offset(l+w,y),grid);txt.text=TextSpan(text:'${(v*100).round()}%',style:TextStyle(fontSize:8,color:scheme.onSurfaceVariant));txt.layout();txt.paint(canvas,Offset(0,y-5));}final ss=<List<double?>>[List.generate(sessions.length,(i)=>after(sessions[i],i,'reading')),List.generate(sessions.length,(i)=>after(sessions[i],i,'hands')),List.generate(sessions.length,(i)=>after(sessions[i],i,'memory')),List.generate(sessions.length,(i)=>after(sessions[i],i,'interpretation'))];final pp=<Paint>[Paint()..color=scheme.primary..strokeWidth=2.4,Paint()..color=scheme.secondary..strokeWidth=2.4,Paint()..color=scheme.tertiary..strokeWidth=2.4,Paint()..color=scheme.error..strokeWidth=2.4];for(var k=0;k<ss.length;k++){Offset? prev;for(var i=0;i<ss[k].length;i++){final v=ss[k][i];if(v==null){prev=null;continue;}final x=l+(sessions.length<=1?w/2:w*i/(sessions.length-1));final y=t+h*(1-v.clamp(0,1));final q=Offset(x,y);if(prev!=null)canvas.drawLine(prev,q,pp[k]);canvas.drawCircle(q,3,Paint()..color=pp[k].color);prev=q;}}}
  @override bool shouldRepaint(covariant _StageEvolutionPainter old)=>true;
}

class _TempoEvolutionPainter extends CustomPainter {
  _TempoEvolutionPainter(this.sessions,this.project,this.targetTempo,this.scheme); final List<Session> sessions; final Project project; final int targetTempo; final ColorScheme scheme;
  int? afterTempo(int i){final direct=sessions[i].newTempo;if(direct!=null&&direct>0)return direct;if(i+1<sessions.length){final p=sessions[i+1].previousTempo;if(p!=null&&p>0)return p;}return project.currentTempo>0?project.currentTempo:null;}
  @override void paint(Canvas canvas,Size size){const l=34.0,r=8.0,t=8.0,b=18.0;final values=List.generate(sessions.length,afterTempo).whereType<int>().where((v)=>v>0).toList();if(values.isEmpty)return;final minV=math.max(0,((values.reduce(math.min)*.9)~/5)*5);final raw=targetTempo>0?math.max(targetTempo,values.reduce(math.max)):values.reduce(math.max);final maxV=math.max(minV+20,((raw*1.08).ceil()~/5)*5);final w=size.width-l-r,h=size.height-t-b;final grid=Paint()..color=scheme.outlineVariant..strokeWidth=1;final txt=TextPainter(textDirection:TextDirection.ltr);for(var j=0;j<=2;j++){final y=t+h*(1-j/2);canvas.drawLine(Offset(l,y),Offset(l+w,y),grid);final v=minV+(maxV-minV)*j/2;txt.text=TextSpan(text:'${v.round()}',style:TextStyle(fontSize:8,color:scheme.onSurfaceVariant));txt.layout();txt.paint(canvas,Offset(0,y-5));}if(targetTempo>0){final y=t+h*(1-((targetTempo-minV)/(maxV-minV)).clamp(0,1));canvas.drawLine(Offset(l,y),Offset(l+w,y),Paint()..color=scheme.error.withOpacity(.55)..strokeWidth=1.5);}final line=Paint()..color=scheme.primary..strokeWidth=2.5;Offset? prev;for(var i=0;i<sessions.length;i++){final v=afterTempo(i);if(v==null){prev=null;continue;}final x=l+(sessions.length<=1?w/2:w*i/(sessions.length-1));final y=t+h*(1-((v-minV)/(maxV-minV)).clamp(0,1));final q=Offset(x,y);if(prev!=null)canvas.drawLine(prev,q,line);canvas.drawCircle(q,3,Paint()..color=line.color);prev=q;}}
  @override bool shouldRepaint(covariant _TempoEvolutionPainter old)=>true;
}
