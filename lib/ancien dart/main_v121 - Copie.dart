...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  void _adaptRemainingWeekToReality() {
    ...
    for (final item in future) {
      ...
      final minMinutes = 10;
      final maxMinutes = 30;
      final adjusted = (item.plannedDuration * factor).round().clamp(minMinutes, maxMinutes).toInt();
      if (adjusted == item.plannedDuration) continue;
      item.plannedDuration = adjusted;
      // Une séance future n'a pas encore de durée réalisée : on garde la durée
      // affichée alignée sur le nouveau plan tant qu'elle n'est pas commencée.
