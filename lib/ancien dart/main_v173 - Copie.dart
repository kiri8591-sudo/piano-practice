...
class CoachHome extends StatelessWidget {
  ...
  /// V173 : signale une stagnation récente utilisable directement par le moteur
  /// du coach. On exige au moins deux séances dans les 30 derniers jours et
  /// aucune amélioration significative entre la première et la dernière.
  bool _isStagnantForCoach(Project p) {
    ...
    final stageDeltas = <double>[
      (last.newReading ?? 0) - (first.newReading ?? 0),
      (last.newHandsTogether ?? 0) - (first.newHandsTogether ?? 0),
      (last.newMemory ?? 0) - (first.newMemory ?? 0),
      (last.newInterpretation ?? 0) - (first.newInterpretation ?? 0),
    ];
    final maxStageDelta = stageDeltas.fold<double>(0, (m, d) => d > m ? d : m);
    final tempoDelta = first.newTempo != null && last.newTempo != null
        ? last.newTempo! - first.newTempo!
        : 0;
    return maxStageDelta < .05 && tempoDelta < 2;
  }
