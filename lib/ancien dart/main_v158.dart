// V158 — journal coach fiable : morceau analysé et morceau réellement ajusté toujours distincts et persistés.
...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  void _recordCoachDecision(Session session, bool changed) {
    ...
    final decision = CoachDecision(
      ...
      message: changed
          ? 'J’ai analysé ta séance sur ${analyzedName ?? 'morceau non renseigné'} et ajusté le planning à venir : ${adjustedName ?? 'morceau du planning'}${adjustedDate == null ? '' : ' le ${_coachPlanDateLabel(adjustedDate)}'}${_lastCoachOldDuration != null && _lastCoachNewDuration != null ? ', ${_lastCoachOldDuration} → ${_lastCoachNewDuration} min' : ''}.'
