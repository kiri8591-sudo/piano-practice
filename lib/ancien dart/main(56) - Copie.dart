...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  Future<void> startRunThrough(Project project) async {
    ...
    final result = await showDialog<_RunThroughResult>(
      context: navKey.currentContext!,
      builder: (_) => const _RunThroughResultDialog(),
    );
...
class _RunThroughResult {
  const _RunThroughResult({required this.completed, required this.stopNote});
  final bool completed;
  final String stopNote;
}

class _RunThroughResultDialog extends StatefulWidget {
  const _RunThroughResultDialog();
  @override
...
class _RunThroughResultDialogState extends State<_RunThroughResultDialog> {
  ...
  Widget build(BuildContext context) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () => Navigator.pop(context, _RunThroughResult(completed: completed, stopNote: noteCtrl.text)),
...
class _ProjectCoachDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (last?.type == 'Run-through' && last?.runThroughCompleted == false) {
      ...
      nextText = 'Run-through terminé : consolide la continuité et vise une exécution de plus en plus régulière.';
    } else if (difficult >= 2) {
