      ...
      if (rec.isNotEmpty) {
        ...
        while (madeProgress) {
          ...
          for (final day in days) {
            ...
            // Pour une journée comportant plusieurs morceaux, on évite qu'un seul
            // morceau absorbe tout le temps disponible. Un morceau est plafonné à
            // 40 min/jour tant qu'il existe au moins un autre morceau à travailler.
            // Si un seul morceau reste disponible, il peut naturellement utiliser
            // tout le temps restant de la journée.
            final availableProjects = orderedIds.where((id) => (remainingByProject[id] ?? 0) > 0).toList();
            final cappedProjects = availableProjects.where((id) {
              final key = '${day.toIso8601String()}|$id';
              return (itemByDayAndProject[key]?.duration ?? 0) < 40;
            }).toList();
            final candidates = availableProjects.length > 1 ? cappedProjects : availableProjects;
            ...
            final dailyProjectCap = availableProjects.length > 1 ? 40 : remainingByDay[day]!;
            final roomForProject = (dailyProjectCap - alreadyToday).clamp(0, dailyProjectCap);
            final chunk = [remainingByProject[projectId]!, remainingByDay[day]!, roomForProject]
                .reduce((a, b) => a < b ? a : b);
            if (chunk <= 0) continue;
