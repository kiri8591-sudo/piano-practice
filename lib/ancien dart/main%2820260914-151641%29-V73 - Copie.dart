...
class ProgressDashboardScreen extends StatelessWidget {
  ...
  Widget build(BuildContext c) {
    ...
    final recent7 = recent.where((s) => DateTime.now().difference(s.date).inDays < 7).toList();
      ...
      body: ListView(
        ...
        children: [
          ...
          if (ranked.isNotEmpty) ...[
            const SizedBox(height: 16),
            CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              _sectionHeader(c, Icons.psychology_outlined, 'Ce que le coach ferait maintenant', subtitle: 'Les morceaux qui méritent le plus d’attention selon leur avancement et leur besoin actuel.'),
              const SizedBox(height: 12),
              for (final p in ranked.take(4))
                Padding(padding: const EdgeInsets.only(bottom: 11), child: Row(children: [
                  Text(p.emoji, style: const TextStyle(fontSize: 22)),
                  const SizedBox(width: 9),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(p.name, style: const TextStyle(fontWeight: FontWeight.w700)),
                    const SizedBox(height: 3),
                    Text('${(p.progress * 100).round()} % · ${p.effectiveWorkFocus}', style: TextStyle(fontSize: 11, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                  ])),
