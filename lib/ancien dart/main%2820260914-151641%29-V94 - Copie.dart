        ...
        builder: (c, setD) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text(
                  'Les seances non terminees des 7 prochains jours seront remplacees. Ta routine reserve son temps en premier chaque jour concerne, puis le reste est reparti entre tes morceaux selon leur avancement, leur besoin réel et une rotation quotidienne pour favoriser la variété.',
                ),
...
class CoachHome extends StatelessWidget {
        ...
        Container(
          ...
          decoration: BoxDecoration(
            gradient: LinearGradient(
              colors: [
                Theme.of(c).colorScheme.primaryContainer.withOpacity(.88),
                Theme.of(c).colorScheme.surfaceContainerHighest.withOpacity(.72),
              ],
            ),
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: Theme.of(c).colorScheme.primary.withOpacity(.14)),
          ),
          child: Row(children: [
            Container(width: 46, height: 46, alignment: Alignment.center, decoration: BoxDecoration(color: Theme.of(c).colorScheme.surface.withOpacity(.82), borderRadius: BorderRadius.circular(14)), child: Icon(Icons.auto_awesome, color: Theme.of(c).colorScheme.primary)),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(height: 3),
              const Text('Une séance utile, maintenant', style: TextStyle(fontSize: 19, fontWeight: FontWeight.w800)),
            ])),
            if (todayItems.isNotEmpty) ...[
              const SizedBox(width: 10),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
                decoration: BoxDecoration(color: Theme.of(c).colorScheme.surface.withOpacity(.82), borderRadius: BorderRadius.circular(12)),
                child: Text('${todayItems.where((x) => x.completed).length}/${todayItems.length}', style: TextStyle(fontWeight: FontWeight.w900, color: Theme.of(c).colorScheme.primary)),
              ),
            ],
          ]),
        ),
  ...
  Widget _stat(BuildContext c, IconData icon, String label, String value, double ratio) => CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [
      Container(width: 34, height: 34, alignment: Alignment.center, decoration: BoxDecoration(color: Theme.of(c).colorScheme.primaryContainer.withOpacity(.72), borderRadius: BorderRadius.circular(11)), child: Icon(icon, size: 18, color: Theme.of(c).colorScheme.primary)),
      const SizedBox(width: 9),
      Expanded(child: Text(label, style: const TextStyle(fontWeight: FontWeight.w800))),
      Text('${(ratio * 100).round()}%', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w900, color: Theme.of(c).colorScheme.primary)),
    ]),
