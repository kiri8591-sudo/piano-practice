...
class _WeekState extends State<Week> {
  ...
  bool _wasAdjustedByCoach(PlanItem item) {
    // plannedDuration = durée initialement prévue ; duration = durée actuellement
    // proposée. Une différence sur une séance future signale un recalibrage.
    return !item.completed &&
        item.sourceSessionId == null &&
        item.duration > 0 &&
        item.plannedDuration > 0 &&
        item.duration != item.plannedDuration;
  }
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Text(
              'Planning ajusté par le coach : $adjusted séance${adjusted > 1 ? 's' : ''} future${adjusted > 1 ? 's' : ''} recalibrée${adjusted > 1 ? 's' : ''} selon ta pratique récente.',
              style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w700, color: Theme.of(c).colorScheme.onSurface),
            ),
          ),
        ]),
      ),
  ...
  Widget _coachAdjustmentLabel(BuildContext c, PlanItem item) {
    ...
    final text = delta < 0
        ? '🔄 Coach : ${item.duration} min au lieu de ${item.plannedDuration} min'
        : '🔄 Coach : ${item.duration} min au lieu de ${item.plannedDuration} min';
    return Padding(
      padding: const EdgeInsets.only(top: 5),
      child: Text(
        text,
        style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: Theme.of(c).colorScheme.primary),
      ),
    );
  }
