    ...
    if (rec.isEmpty && routine.isEmpty) {
      await showDialog<void>(
        ...
        builder: (c) => AlertDialog(
          ...
          content: const Text('Ajoute au moins un morceau (projet) dont l\'avancement n\'est pas terminé, ou une entrée dans "Ma routine", pour générer une proposition.'),
          actions: [FilledButton(onPressed: () => Navigator.pop(c), child: const Text('Ok'))],
        ),
      );
      return;
...
class CardBox extends StatelessWidget {
  ...
  Widget build(BuildContext c) => Card(
        ...
        elevation: 0,
        color: Theme.of(c).colorScheme.surfaceContainerHigh,
        shape: RoundedRectangleBorder(
          borderRadius: BorderRadius.circular(20),
          side: BorderSide(color: Theme.of(c).colorScheme.outlineVariant.withOpacity(.35)),
        ),
      ...
      children: [
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 8),
            Expanded(
              child: FilledButton.tonalIcon(onPressed: onQuickProject, icon: const Icon(Icons.add_circle_outline), label: const Text('Projet')),
            ),
          ],
        ),
        const SizedBox(height: 24),
                              ...
                              if (s.instruction != null)
                                Align(
                                  ...
                                  child: TextButton.icon(
                                    ...
                                    label: const Text('Orienter la semaine prochaine'),
                                  ),
                                ),
                            ],
                          ),
                  ...
                  children: [
                    ...
                    ...todayItems.map((item) => Padding(
                          ...
                          child: Row(children: [
                            ...
                            if (item.duration > 0) Text('${item.duration} min'),
                          ]),
                        )),
                    const SizedBox(height: 6),
                    FilledButton.icon(
        ...
        if (projects.isNotEmpty) ...[
          const Text('Mes projets', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          const SizedBox(height: 10),
...
class _ProjectsState extends State<Projects> {
  ...
  Widget build(BuildContext c) {
    ...
    return Column(
      children: [
        AppBar(title: const Text('Projets'), actions: [IconButton(onPressed: widget.onAdd, icon: const Icon(Icons.add))]),
        Padding(
        ...
        Expanded(
          child: widget.items.isEmpty
              ? emptyState('Aucun projet pour l\'instant.\nAppuie sur + pour en creer un.')
              : ListView.separated(
                            ...
                            children: [
                              Row(children: [
                                ...
                                IconButton(
                                  ...
                                  tooltip: 'Supprimer',
                                  onPressed: () async {
                                    if (await confirmDelete(c, 'ce projet')) widget.onDelete(p);
                                  },
                                ),
                              ]),
                              const SizedBox(height: 8),
                              Wrap(spacing: 8, runSpacing: 6, children: [
                                Chip(
                                  visualDensity: VisualDensity.compact,
                                  avatar: Text(projectStatusEmoji(p.effectiveStatus), style: const TextStyle(fontSize: 15)),
                                  label: Text(p.effectiveStatus, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                                ),
                              ]),
                              const SizedBox(height: 4),
                              progress(p.progress),
                              const SizedBox(height: 8),
                              Text(p.goal, style: const TextStyle(color: Colors.grey)),
                              const SizedBox(height: 12),
                              Wrap(spacing: 8, runSpacing: 8, children: [
                                _miniProgress('Lecture', p.reading),
                                _miniProgress('Mains ens.', p.handsTogether),
                                _miniProgress('Mémoire', p.memory),
                                _miniProgress('Interprétation', p.interpretation),
                              ]),
                              if (p.currentTempo > 0 || p.targetTempo > 0 || p.measures.isNotEmpty) ...[
                                const SizedBox(height: 10),
                                Wrap(spacing: 12, children: [
                                  if (p.measures.isNotEmpty) Text('Mesures : ${p.measures}', style: const TextStyle(fontSize: 12)),
                                  if (p.currentTempo > 0 || p.targetTempo > 0) Text('Tempo : ${p.currentTempo} / ${p.targetTempo} BPM', style: const TextStyle(fontSize: 12)),
                                ]),
                              ],
...
Widget _progressSlider(String label, double value, ValueChanged<double> onChanged) => Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(children: [Expanded(child: Text(label, style: const TextStyle(fontSize: 12))), Text('${(value * 100).round()}%', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold))]),
...
class _ProjectDialogState extends State<ProjectDialog> {
  ...
  Widget build(BuildContext c) {
    final editing = widget.existing != null;
    return AlertDialog(
      title: Text(editing ? 'Modifier le projet' : 'Nouveau projet'),
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            ...
            _progressSlider('Interprétation', interpretation, (v) => setState(() => interpretation = v)),
            Row(children: [
              Expanded(child: TextField(controller: measuresCtrl, decoration: const InputDecoration(labelText: 'Mesures travaillées', hintText: 'ex. 25–48'))),
              const SizedBox(width: 10),
              Expanded(child: TextField(controller: currentTempoCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Tempo actuel'))),
              const SizedBox(width: 10),
              Expanded(child: TextField(controller: targetTempoCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Tempo cible'))),
            ]),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (cats.isNotEmpty) ...[
                      ...
                      const SizedBox(height: 4),
                      const Text(
                        'Basée sur l\'avancement de tes morceaux : les moins avancés (et les projets prioritaires) reçoivent plus de temps.',
