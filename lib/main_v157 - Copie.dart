// V157 — journal coach fiable : morceau analysé et morceau réellement ajusté toujours distincts et persistés.
...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  void _recordCoachDecision(Session session, bool changed) {
    ...
    final decision = CoachDecision(
      ...
      message: changed
          ? 'Séance analysée : ${analyzedName ?? 'morceau non renseigné'}. Planning ajusté : ${adjustedName ?? 'morceau du planning'}${adjustedDate == null ? '' : ' le ${_coachPlanDateLabel(adjustedDate)}'}.${adjustedTitle != null && adjustedTitle.trim().isNotEmpty && adjustedTitle != adjustedName ? ' ${adjustedTitle!}.' : ''}'
          ...
          child: decisions.isEmpty
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    final analyzedName = d.analyzedProjectName ?? projectById(d.projectId)?.name;
                    final adjustedName = d.adjustedProjectName ?? projectById(d.adjustedProjectId)?.name;
                    final target = d.changed
                        ? 'Séance analysée : ${analyzedName ?? 'morceau non renseigné'}\nPlanning ajusté : ${adjustedName ?? 'morceau du planning'} · ${_coachDecisionTargetLabel(d)}'
                        : (analyzedName == null ? '' : 'Séance analysée : $analyzedName');
                    final details = [
                      date,
                      if (target.isNotEmpty) target,
                      if (d.changed && d.adjustedPlanTitle != null && d.adjustedPlanTitle!.trim().isNotEmpty && d.adjustedPlanTitle != adjustedName) 'Élément : ${d.adjustedPlanTitle}',
                      if (d.reason == null || d.reason!.trim().isEmpty) d.message,
  ...
  Future<void> _showCoachPlanningResult(Session session, bool changed) async {
    ...
    final adjustedDateText = adjustedDate == null ? '' : _coachPlanDateLabel(adjustedDate);

    await showDialog<void>(
      context: navKey.currentContext!,
      builder: (c) => AlertDialog(
        title: Text(changed ? '🧠 Analyse du coach' : '🧠 Analyse du coach'),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Séance analysée', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
            const SizedBox(height: 3),
            Text(
              '${analyzedProject?.emoji ?? '🎹'} $analyzedName',
              style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 17),
            ),
            const SizedBox(height: 10),
            Text(
              changed
                  ? 'J’ai analysé ta séance sur $analyzedName et ajusté le planning à venir.'
                  : 'J’ai analysé ta séance sur $analyzedName. Rien ne justifie de changer le planning.',
              style: const TextStyle(height: 1.4),
            ),
            if (changed && adjustedItem != null) ...[
              const SizedBox(height: 14),
              const Text('Planning ajusté', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
              const SizedBox(height: 3),
              Text(
                '${adjustedProject?.emoji ?? '🎹'} $adjustedName${adjustedDateText.isEmpty ? '' : ' · $adjustedDateText'}',
                style: const TextStyle(fontWeight: FontWeight.w800),
              ),
              if (_lastCoachOldDuration != null && _lastCoachNewDuration != null) ...[
                const SizedBox(height: 3),
              ...
              const SizedBox(height: 10),
              Text(
                adjustedItem.projectId == session.projectId
                    ? 'Le planning de ce même morceau a été ajusté.'
                    : 'Le morceau ajusté peut être différent : le coach adapte aussi la charge globale et la variété de la semaine.',
                style: TextStyle(color: Theme.of(c).colorScheme.onSurfaceVariant, fontSize: 12, height: 1.35),
              ),
            ] else if (changed) ...[
              const SizedBox(height: 10),
              const Text('Le planning a été ajusté, mais le morceau concerné n’est plus disponible dans les données actuelles.', style: TextStyle(fontSize: 12)),
            ],
            if (changed) ...[
              const SizedBox(height: 10),
              const Text('Un indicateur COACH apparaît directement sur la séance modifiée dans le planning.', style: TextStyle(fontSize: 12)),
            ],
          ],
        ),
        actions: [
