// V263 — correction de compilation : SizedBox dynamique dans Maîtrise des morceaux.
...
// V191 — espaces réguliers entre champs et sections.
Widget _dialogFieldGap(BuildContext context, {double phone = 12, double desktop = 6}) =>
    SizedBox(height: _isPhoneLayout(context) ? phone : desktop);

Widget _dialogSectionGap(BuildContext context, {double phone = 18, double desktop = 12}) =>
    SizedBox(height: _isPhoneLayout(context) ? phone : desktop);
...
InputDecorationThemeData _mobileFriendlyDialogInputs(BuildContext context) {
  ...
  return base.copyWith(
    border: OutlineInputBorder(
      ...
      borderSide: BorderSide(color: scheme.outlineVariant.withOpacity(.50), width: 1),
    ),
    enabledBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(13),
      borderSide: BorderSide(color: scheme.outlineVariant.withOpacity(.58), width: 1),
    ),
    ...
    errorBorder: OutlineInputBorder(
      borderRadius: BorderRadius.circular(13),
      borderSide: BorderSide(color: scheme.error.withOpacity(.78), width: 1.2),
    ),
    ...
    filled: true,
    fillColor: scheme.surfaceContainerHighest.withOpacity(.58),
    isDense: false,
...
Widget _detailSectionHeader(BuildContext context, String title, {IconData? icon}) {
  ...
  final scheme = Theme.of(context).colorScheme;
  return Padding(
    padding: EdgeInsets.only(top: phone ? 16 : 14, bottom: phone ? 9 : 6),
    child: Column(
      children: [
        Row(
          children: [
            if (icon != null) ...[
              Container(
                ...
                decoration: BoxDecoration(
                  color: scheme.primaryContainer.withOpacity(phone ? .60 : .42),
                  borderRadius: BorderRadius.circular(phone ? 9 : 8),
                ),
                child: Icon(icon, size: phone ? 17 : 16, color: scheme.primary),
              ),
        ...
        if (phone) ...[
          ...
          Divider(height: 1, color: scheme.outlineVariant.withOpacity(.52)),
        ],
      ],
    ),
  );
...
double runThroughMasteryScore(Project p, List<Session> sessions) {
  ...
  // V163 : l'indicateur ne dépend plus d'un seul run. On regarde jusqu'à
  // trois exécutions terminées afin d'évaluer une maîtrise réelle et stable.
  final selected = runs.take(3).toList();
  double scoreFor(Session run) {
    final end = run.runThroughEndTempo ?? 0;
    final start = run.runThroughStartTempo ?? 0;
    final tempoScore = p.targetTempo > 0 && end > 0
        ? (end / p.targetTempo).clamp(0.0, 1.0)
        : 1.0;
    final continuityScore = 1.0;
    final stabilityScore = start > 0 && end > 0
        ? (end / start).clamp(0.0, 1.0)
        : 0.80;
    return tempoScore * .55 + continuityScore * .25 + stabilityScore * .20;
  }

  // Le dernier run compte davantage, mais les précédents empêchent une
  // amélioration ponctuelle de donner immédiatement une impression de maîtrise.
  const weights = [0.50, 0.30, 0.20];
  var weighted = 0.0;
  var weightTotal = 0.0;
  for (var i = 0; i < selected.length; i++) {
    weighted += scoreFor(selected[i]) * weights[i];
    weightTotal += weights[i];
  }
  return ((weighted / weightTotal).clamp(0.0, 1.0) * 100).roundToDouble();
}
...
class CoachDecision {
  CoachDecision({
    ...
    this.analyzedProjectName,
    this.adjustedProjectName,
    this.adjustedPlanTitle,
    this.analyzedSessionDate,
    this.adjustedAt,
    this.adjustmentType,
    this.oldFocus,
    this.newFocus,
  ...
  /// Instantanés des libellés au moment de la décision. Ils évitent qu'une
  /// modification ultérieure des fiches ne fasse apparaître un mauvais morceau
  /// dans le journal du coach.
  final String? analyzedProjectName;
  final String? adjustedProjectName;
  final String? adjustedPlanTitle;
  /// Date/heure de la séance réellement analysée.
  final DateTime? analyzedSessionDate;
  /// Date/heure à laquelle le coach a effectivement modifié l’élément du planning.
  final DateTime? adjustedAt;
  /// Type d'adaptation : duration, focus ou none.
  ...
  Map<String, dynamic> toJson() => {
        ...
        'analyzedProjectName': analyzedProjectName,
        'adjustedProjectName': adjustedProjectName,
        'adjustedPlanTitle': adjustedPlanTitle,
        'analyzedSessionDate': analyzedSessionDate?.toIso8601String(),
  ...
  factory CoachDecision.fromJson(Map<String, dynamic> j) => CoachDecision(
        ...
        analyzedProjectName: j['analyzedProjectName'] as String?,
        adjustedProjectName: j['adjustedProjectName'] as String?,
        adjustedPlanTitle: j['adjustedPlanTitle'] as String?,
        analyzedSessionDate: DateTime.tryParse(j['analyzedSessionDate']?.toString() ?? ''),
        adjustedAt: DateTime.tryParse(j['adjustedAt']?.toString() ?? ''),
...
class PlanItem {
  ...
  factory PlanItem.fromJson(Map<String, dynamic> j) {
    ...
    final plannedRaw = (j['plannedDuration'] as num?)?.toInt() ?? duration;
    final planned = plannedRaw < 0 ? 0 : plannedRaw;
...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  // V160 : focus effectif utilise par le moteur d'adaptation du coach.
  // Le moteur vit dans _PianoPracticeAppState ; il ne peut pas appeler
  ...
  String? _lastCoachAdjustmentType;
  String? _lastCoachOldFocus;
  String? _lastCoachNewFocus;
  ...
  WeeklyBilan weeklyBilanFor(DateTime weekStart) {
    ...
    final duePlan = weekPlan.where((x) => !x.date.isAfter(todayOnly)).toList();
    final upcomingPlan = weekPlan.where((x) => x.date.isAfter(todayOnly)).toList();

    // Pour la comparaison prévue → réalisée, on ne met pas les séances futures
    // au dénominateur. Sinon, en milieu de semaine, le taux paraît artificiellement
    // faible alors que ces séances n'étaient simplement pas encore dues.
    int plannedForItem(PlanItem item) {
      final source = item.sourceSessionId == null
          ? null
          : sessions.where((s) => s.id == item.sourceSessionId).firstOrNull;
      return source?.plannedDuration ?? (item.plannedDuration > 0 ? item.plannedDuration : item.duration);
    }

    final duePlannedMinutes = duePlan.fold(0, (a, x) => a + plannedForItem(x));
    final upcomingPlannedMinutes = upcomingPlan.fold(0, (a, x) => a + (x.duration > 0 ? x.duration : x.plannedDuration));
    final plannedMinutes = duePlannedMinutes + upcomingPlannedMinutes;
    final plannedSessions = weekPlan.where((x) => (x.plannedDuration > 0 || x.duration > 0)).length;
    final duePlannedSessions = duePlan.where((x) => (x.plannedDuration > 0 || x.duration > 0)).length;
    final upcomingPlannedSessions = upcomingPlan.where((x) => (x.plannedDuration > 0 || x.duration > 0)).length;
    final completedPlannedSessions = duePlan.where((x) => x.completed && x.sourceSessionId != null).length;
    final realizedPlannedMinutes = duePlan.fold(0, (a, x) {
      final source = x.sourceSessionId == null ? null : sessions.where((s) => s.id == x.sourceSessionId).firstOrNull;
      return a + (source?.duration ?? 0);
    });

    final planComparisons = weekPlan.map((x) {
      final source = x.sourceSessionId == null
          ? null
          : sessions.where((s) => s.id == x.sourceSessionId).firstOrNull;
      final planned = plannedForItem(x);
      final realized = source?.duration;
      final project = x.projectId == null ? null : projectById(x.projectId);
      return WeeklyPlanComparison(
        planItemId: x.id,
        date: x.date,
        projectName: project?.name ?? x.title,
        plannedMinutes: planned,
        realizedMinutes: realized,
        completed: x.completed && source != null,
        coachAdjusted: x.coachAdjusted,
        coachAdjustmentType: x.coachAdjustmentType,
      );
    }).toList();
    ...
    return WeeklyBilan(
      ...
      plannedMinutes: plannedMinutes,
      duePlannedMinutes: duePlannedMinutes,
      upcomingPlannedMinutes: upcomingPlannedMinutes,
      realizedPlannedMinutes: realizedPlannedMinutes,
      plannedSessions: plannedSessions,
      duePlannedSessions: duePlannedSessions,
      upcomingPlannedSessions: upcomingPlannedSessions,
  ...
  Future<void> importData() async {
    // iPhone/iPad Safari : le sélecteur de fichiers est plus fiable lorsque
    // l'input est temporairement monté dans le DOM et réellement présent dans
    // la page (sans display:none).
    ...
    try {
      ...
      try {
        ...
        await showDialog<void>(
          ...
          builder: (c) => AlertDialog(
            ...
            content: const Text('Ce fichier ne semble pas être une sauvegarde valide de Piano Practice.'),
            actions: [
      ...
      final ok = await showDialog<bool>(
        ...
        builder: (c) => AlertDialog(
          ...
          content: const Text(
            'Toutes les données actuelles (morceaux, sessions, routine, planning) seront remplacées. Avant de continuer, il est recommandé de créer une sauvegarde de sécurité.',
          ),
          actions: [
            TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('Annuler')),
            TextButton(
      ...
      try {
        setState(() {
          dailyCapacity = ((data['dailyCapacity'] as List?) ?? [30, 30, 45, 30, 30, 45, 30])
          ...
          if (data['learningMethods'] is List) {
            learningMethods = (data['learningMethods'] as List)
                .map((e) => e.toString())
          ...
          bestStreakEver = (data['bestStreakEver'] as num?)?.toInt() ?? 0;
          seenBadgeIds = ((data['seenBadgeIds'] as List?) ?? []).map((e) => e.toString()).toList();
  ...
  // V222 : l'ordre des créneaux d'une même journée doit lui aussi varier.
  // Il ne s'agit pas d'ajouter des séances : on réordonne uniquement les éléments
  // générés pour que le premier travail actif ne soit pas systématiquement le même.
  // Les routines utilisateur restent en tête, tandis qu'un Run-through est conservé
  // en fin de séquence comme vérification globale. Les focus manuels ne sont jamais modifiés.
  void _applyCoachDayOrder(List<DateTime> days) {
    ...
    int focusIndex(String focus) {
      final index = focusOrder.indexOf(focus);
      return index < 0 ? focusOrder.length : index;
    }

    String itemFocus(PlanItem item) {
      if (item.category == 'Run-through') return 'Run-through';
      if (item.projectId == null) return 'Routine';
      // Une variation générée par V221 est visible dans les détails du créneau.
      // On la prend en compte pour que l'alternance affichée corresponde au vrai focus.
      for (final focus in focusOrder) {
        if (item.details.contains('variation') && item.details.contains(' $focus')) {
          return focus;
        }
      }
      final project = projectById(item.projectId);
      if (project?.effectiveStatus == 'Répertoire d’entretien') return 'Entretien';
      return project?.effectiveWorkFocus.trim().isNotEmpty == true
          ? project!.effectiveWorkFocus
          : 'Consolidation';
    }

    for (final day in days) {
      final dayItems = plan.where((x) => !x.completed && _sameDay(x.date, day)).toList();
      if (dayItems.length < 2) continue;

      final routine = dayItems.where((x) => x.projectId == null && x.category != 'Run-through').toList();
      final runThrough = dayItems.where((x) => x.category == 'Run-through' && x.projectId != null).toList();
      final active = dayItems
          .where((x) => x.projectId != null && x.category != 'Run-through')
          .toList();

      if (active.length < 2) continue;

      final represented = active
          .map(itemFocus)
          .where((focus) => focus != 'Run-through' && focus != 'Routine')
          .toSet()
          .toList()
        ..sort((a, b) => focusIndex(a).compareTo(focusIndex(b)));
      if (represented.length < 2) continue;

      final startFocus = represented[(day.weekday - 1) % represented.length];
      int circularDistance(String focus) {
        final start = focusIndex(startFocus);
        final current = focusIndex(focus);
        if (current >= focusOrder.length) return focusOrder.length + 5;
        return (current - start + focusOrder.length) % focusOrder.length;
      }

      final indexed = active.asMap().entries.toList();
      indexed.sort((a, b) {
        final focusA = itemFocus(a.value);
        final focusB = itemFocus(b.value);
        final byDistance = circularDistance(focusA).compareTo(circularDistance(focusB));
        if (byDistance != 0) return byDistance;
        final projectA = a.value.projectId == null ? null : projectById(a.value.projectId);
        final projectB = b.value.projectId == null ? null : projectById(b.value.projectId);
        final priorityA = projectA?.priority == true ? 1 : 0;
        final priorityB = projectB?.priority == true ? 1 : 0;
        if (priorityA != priorityB) return priorityB.compareTo(priorityA);
        return a.key.compareTo(b.key);
      });

      final reorderedActive = indexed.map((e) => e.value).toList();
      final reordered = <PlanItem>[...routine, ...reorderedActive, ...runThrough];

      // Remplace uniquement la position des éléments concernés dans la journée.
      var cursor = 0;
      for (var i = 0; i < plan.length; i++) {
        if (plan[i].completed || !_sameDay(plan[i].date, day)) continue;
        if (cursor >= reordered.length) break;
        plan[i] = reordered[cursor++];
      }
    }
  }
  ...
  Future<void> proposeWeeklyPlan({bool skipInstructionsDialog = false}) async {
    ...
    // V223 : lorsqu'un signal de récupération est objectivement présent, le coach
    // réserve une seule journée plus légère. La capacité déclarée par l'utilisateur
    // n'est jamais modifiée : on laisse simplement une partie du temps disponible
    // sans fabriquer de séances supplémentaires.
    var recentActualMinutes = 0;
    var recentCapacityMinutes = 0;
    var recentDifficultCount = 0;
    for (var i = 0; i < 4; i++) {
      final d = today.subtract(Duration(days: i));
      recentCapacityMinutes += dailyCapacity[d.weekday - 1];
      for (final s in sessions) {
        final sd = DateTime(s.date.year, s.date.month, s.date.day);
        if (_sameDay(sd, d) && !s.date.isAfter(now)) {
          recentActualMinutes += s.duration;
          if (s.coachFeeling == 'difficile') recentDifficultCount++;
        }
      }
    }
    final recentLoadRatio = recentCapacityMinutes > 0
        ? recentActualMinutes / recentCapacityMinutes
        : 0.0;
    final recoverySignal =
        recentDifficultCount >= 2 ||
        (recentActualMinutes >= 90 && recentLoadRatio >= .90);

    DateTime? coachLightDay;
    if (recoverySignal) {
      final candidates = days
          .where((d) => !d.isBefore(today) && dailyCapacity[d.weekday - 1] >= 30)
          .toList();
      final afterToday = candidates.where((d) => !_sameDay(d, today)).toList();
      final pool = afterToday.isNotEmpty ? afterToday : candidates;
      if (pool.isNotEmpty) {
        pool.sort((a, b) {
          final capA = dailyCapacity[a.weekday - 1];
          final capB = dailyCapacity[b.weekday - 1];
    ...
    setState(() {
      ...
      if (rec.isNotEmpty) {
        ...
        // V221 : mémoire du dernier focus planifié par morceau. Elle sert à
        // éviter de répéter mécaniquement le même angle de travail lorsque le
        // morceau revient plusieurs jours de suite. Un focus manuel reste prioritaire.
        final lastFocusByProject = <String, String>{};
        final recentFocusByProject = <String, List<String>>{};
        ...
        bool projectFocusUsedRecently(String projectId, String focus) {
          return recentFocusByProject[projectId]?.contains(focus) == true;
        }

        bool projectFocusRepeatedYesterday(String projectId, DateTime day, String focus) {
          final lastDay = lastScheduledDayByProject[projectId];
          if (lastDay == null) return false;
          final yesterday = DateTime(day.year, day.month, day.day).subtract(const Duration(days: 1));
          return DateTime(lastDay.year, lastDay.month, lastDay.day) == yesterday &&
              lastFocusByProject[projectId] == focus;
        }

        String automaticAlternateFocus(Project p, String focus) {
          // Ne touche jamais au réglage manuel du morceau : l'alternance porte
          // uniquement sur le créneau généré cette semaine.
          if (p.workFocus != 'Automatique') return focus;
          final alternatives = <String, List<String>>{
            'Déchiffrage': ['Rythme', 'Mains ensemble'],
            'Mains ensemble': ['Passages difficiles', 'Rythme'],
            'Rythme': ['Mains ensemble', 'Passages difficiles'],
            'Mémorisation': ['Mains ensemble', 'Interprétation'],
            'Interprétation': ['Tempo', 'Consolidation'],
            'Tempo': ['Interprétation', 'Consolidation'],
            'Consolidation': ['Passages difficiles', 'Interprétation'],
            'Accords': ['Rythme', 'Passages difficiles'],
            'Passages difficiles': ['Consolidation', 'Mains ensemble'],
            'Entretien': ['Interprétation', 'Tempo'],
          };
          for (final candidate in alternatives[focus] ?? const <String>[]) {
            if (!projectFocusUsedRecently(p.id, candidate)) return candidate;
          }
          return focus;
        }
        ...
        for (final day in days) {
          ...
          while (activeDayBudget >= minUsefulMinutes &&
              ...
              guard++ < 50) {
            final dayRemaining = math.min(remainingByDay[day]!, activeDayBudget);
            ...
            for (final id in untouched) {
              ...
              final isMaintenance = p?.effectiveStatus == 'Répertoire d’entretien';
              final baseFocus = isMaintenance ? 'Entretien' : (p == null ? 'Consolidation' : planningFocus(p));
              final focus = (p == null || p.workFocus != 'Automatique')
                  ? baseFocus
                  : (projectFocusRepeatedYesterday(id, day, baseFocus)
                      ? automaticAlternateFocus(p, baseFocus)
                      : baseFocus);
              final focusMinutes = scheduledMinutesByFocus[focus] ?? 0;
              final focusPenalty = focusMinutes >= 60 ? .76 : (focusMinutes >= 45 ? .88 : 1.0);
              final sameFocusPenalty =
                  p != null && p.workFocus == 'Automatique' && projectFocusRepeatedYesterday(id, day, baseFocus)
                      ? .62
                      : 1.0;
              final recentSameFocusPenalty =
                  p != null && p.workFocus == 'Automatique' && projectFocusUsedRecently(id, focus)
                      ? .84
                      : 1.0;
              final focusDiversityBonus =
                  p != null && p.workFocus == 'Automatique' && !projectFocusUsedRecently(id, focus)
                      ? 1.06
                      : 1.0;
            ...
            final maintenance = project.effectiveStatus == 'Répertoire d’entretien';
            final baseFocus = dueForRunThrough
                ? 'Interprétation'
                : (maintenance ? 'Entretien' : project.effectiveWorkFocus);
            final focus = dueForRunThrough || project.workFocus != 'Automatique'
                ? baseFocus
                : (projectFocusRepeatedYesterday(project.id, day, baseFocus)
                    ? automaticAlternateFocus(project, baseFocus)
                    : baseFocus);
            ...
            final maintenanceDetails = maintenance
                ? '${detailsPrefix}Révision · entretien du répertoire'
                : baseDetails;
            final focusNote = (!dueForRunThrough && project.workFocus == 'Automatique' && focus != project.effectiveWorkFocus)
                ? ' · variation ${workFocusEmoji(focus)} $focus'
                : '';
            final planningDetails = dueForRunThrough
                ...
                : '${maintenanceDetails.isEmpty ? '' : maintenanceDetails}$focusNote';
            ...
            final item = PlanItem(
              ...
              category: dueForRunThrough
                  ? 'Run-through'
                  : (maintenance ? 'Répertoire' : project.weakestCategory),
            ...
            lastFocusByProject[bestId] = focus;
            final focusHistory = recentFocusByProject.putIfAbsent(bestId, () => <String>[]);
            focusHistory.remove(focus);
            focusHistory.insert(0, focus);
            if (focusHistory.length > 3) focusHistory.removeRange(3, focusHistory.length);
      ...
      // V222 : réordonne les créneaux de chaque journée après les deux passes,
      // sans changer les dates ni les durées.
      _applyCoachDayOrder(days);
  ...
  Future<void> addOrEditSession([Session? existing, int? initialDuration]) async {
    ...
    setState(() {
      if (existing == null) {
        ...
        // Une modification de session ne crée pas une nouvelle version
        // d'avancement tant que le morceau reste identique.
        // Si le morceau est changé, les anciens snapshots seraient rattachés
        // au mauvais projet : on recapture donc l'état actuel du nouveau morceau.
        if (existing.projectId != r.projectId) {
          _capturePreviousProgress(r);
          r.newReading = null;
          r.newHandsTogether = null;
          r.newMemory = null;
          r.newInterpretation = null;
          r.newTempo = null;
        } else {
    ...
    // Le ressenti est maintenant enregistré : le coach recalibre les jours
    // futurs uniquement pour une nouvelle séance, pas pour une simple correction
  ...
  void _recordCoachDecision(Session session, bool changed) {
    ...
    final analyzedName = analyzedProject?.name;
    final adjustedName = adjustedProject?.name ?? item?.title;
    final adjustedTitle = item?.title;
    final adjustedAt = item?.coachAdjustedAt ?? (changed ? DateTime.now() : null);
    final decision = CoachDecision(
      ...
      analyzedProjectName: analyzedName,
      adjustedProjectName: changed ? adjustedName : null,
      adjustedPlanTitle: changed ? adjustedTitle : null,
      analyzedSessionDate: session.date,
      adjustedAt: adjustedAt,
      adjustmentType: changed ? _lastCoachAdjustmentType : null,
      oldFocus: changed ? _lastCoachOldFocus : null,
                                  ...
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      if (d.changed) ...[
                                        ...
                                        const SizedBox(height: 2),
                                        Text(
                                          _coachShortReason(hasReason ? d.reason : d.message),
                                          style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, height: 1.35),
                                        ),
                                        if (hasReason && d.reason!.trim() != _coachShortReason(d.reason)) ...[
                                          const SizedBox(height: 4),
                                          Text(
                                            d.reason!,
                                            style: TextStyle(color: Theme.of(c).colorScheme.onSurfaceVariant, fontSize: 11, height: 1.3),
                                          ),
                                        ],
                                      ] else ...[
                                        const SizedBox(height: 8),
  ...
  Future<void> _showCoachPlanningResult(Session session, bool changed) async {
    final analyzedProject = projectById(session.projectId);
    final analyzedName = analyzedProject?.name ?? 'morceau non renseigné';
    final adjustedItem = _lastCoachChangedItemId == null
        ? null
        : plan.where((x) => x.id == _lastCoachChangedItemId).firstOrNull;
    final adjustedProject = adjustedItem == null ? null : projectById(adjustedItem.projectId);
    final adjustedName = adjustedProject?.name ?? adjustedItem?.title ?? 'un morceau du planning';
    final adjustedDate = adjustedItem?.date;
    final adjustedDateText = adjustedDate == null ? 'date non disponible' : _coachPlanDateLabel(adjustedDate);
    final changedAt = adjustedItem?.coachAdjustedAt;
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text('${analyzedProject?.emoji ?? '🎹'} $analyzedName', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 17)),
              const SizedBox(height: 3),
              Text('Réalisée le ${_coachDateTimeLabel(session.date)}', style: const TextStyle(fontSize: 12)),
              const SizedBox(height: 10),
              Text(
                changed
                    ? 'J’ai analysé ta séance sur $analyzedName et j’ai ajusté le planning à venir.'
                    : 'J’ai analysé ta séance sur $analyzedName. Aucun ajustement du planning n’a été nécessaire.',
                style: const TextStyle(height: 1.4),
              ),
              if (changed && adjustedItem != null) ...[
                const SizedBox(height: 14),
                const Text('Modification du planning', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
                const SizedBox(height: 3),
                Text('${adjustedProject?.emoji ?? '🎹'} $adjustedName', style: const TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
                const SizedBox(height: 2),
                Text('Séance concernée : $adjustedDateText', style: const TextStyle(fontSize: 12)),
                if (changedAt != null) Text('Modification effectuée : ${_coachDateTimeLabel(changedAt)}', style: const TextStyle(fontSize: 12)),
                if (_lastCoachAdjustmentType == 'duration' && _lastCoachOldDuration != null && _lastCoachNewDuration != null) ...[
                  const SizedBox(height: 3),
                  Text('Durée : ${_lastCoachOldDuration} → ${_lastCoachNewDuration} min'),
                ] else if (_lastCoachAdjustmentType == 'focus' && _lastCoachOldFocus != null && _lastCoachNewFocus != null) ...[
                  const SizedBox(height: 3),
                  Text('Focus : $_lastCoachOldFocus → $_lastCoachNewFocus'),
                ],
                if ((_lastCoachReason ?? '').trim().isNotEmpty) ...[
                  const SizedBox(height: 8),
                  const Text('Pourquoi ?', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
                  const SizedBox(height: 3),
                  Text(_coachShortReason(_lastCoachReason), style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, height: 1.35)),
                  if ((_lastCoachReason ?? '').trim() != _coachShortReason(_lastCoachReason)) ...[
                    const SizedBox(height: 4),
                    Text(_lastCoachReason!, style: TextStyle(color: Theme.of(c).colorScheme.onSurfaceVariant, fontSize: 11, height: 1.3)),
                  ],
                ],
              ] else if (changed) ...[
                const SizedBox(height: 10),
                const Text('Le planning a été ajusté, mais la séance concernée n’est plus disponible dans les données actuelles.', style: TextStyle(fontSize: 12)),
              ] else if ((_lastCoachReason ?? '').trim().isNotEmpty) ...[
                const SizedBox(height: 9),
                const Text('Pourquoi aucune modification ?', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800)),
                const SizedBox(height: 3),
                Text(_coachShortReason(_lastCoachReason), style: const TextStyle(fontSize: 12.5, fontWeight: FontWeight.w700, height: 1.35)),
                  if ((_lastCoachReason ?? '').trim() != _coachShortReason(_lastCoachReason)) ...[
                    const SizedBox(height: 4),
  ...
  bool _adaptRemainingWeekToReality() {
    ...
    _lastCoachAdjustmentType = null;
    _lastCoachOldFocus = null;
    _lastCoachNewFocus = null;
    ...
    int plannedEvidenceCount(String projectId) {
      final recent = sessions
          .where((s) => s.projectId == projectId &&
              s.plannedDuration != null &&
              s.plannedDuration! > 0)
          .toList();
      return math.min(recent.length, 4);
    }

    int futureMinutesFor(String projectId) {
      return future
          .where((x) => x.projectId == projectId)
          .fold<int>(0, (sum, x) => sum + x.duration);
    }

    int futureCountFor(String projectId) {
      return future.where((x) => x.projectId == projectId).length;
    }

    Set<DateTime> futureDaysFor(String projectId) {
      return future
          .where((x) => x.projectId == projectId)
    ...
    bool weeklyOverconcentration(String projectId) {
      final totalFutureMinutes = future.fold<int>(0, (sum, x) => sum + x.duration);
      if (totalFutureMinutes <= 0) return false;
      final projectMinutes = futureMinutesFor(projectId);
      final projectCount = futureCountFor(projectId);
      final share = projectMinutes / totalFutureMinutes;
      // V220 : une seule pièce ne doit pas récupérer une part disproportionnée
      // du temps restant si d'autres besoins existent dans la semaine.
      return projectCount >= 3 || share >= .50;
    }

    bool focusChangeAllowed(PlanItem item, String newFocus, {bool strongSignal = false}) {
      if (newFocus.trim().isEmpty) return false;
      final lastAt = item.coachAdjustedAt;
      final lastType = item.coachAdjustmentType;
      if (lastAt == null || lastType != 'focus') return true;
      final age = now.difference(lastAt);
      // Une même séance ne doit pas osciller de focus à chaque nouvelle analyse.
      // Même avec un signal fort, on garde au moins 24 h de stabilité ; entre 24 h et 72 h,
      // seul un signal réellement fort (difficile ou stagnation confirmée) peut agir.
      if (age < const Duration(hours: 24)) return false;
      if (age < const Duration(hours: 72) && !strongSignal) return false;
      if (item.coachPreviousFocus != null && item.coachPreviousFocus == newFocus) {
        // Evite le retour immédiat au focus précédent, même lorsque l'analyse est relancée.
        return false;
      }
      return true;
    }
    ...
    double needScore(Project p) {
      ...
      // V220 : le coach regarde aussi la couverture réelle du reste de la semaine.
      // Un morceau prioritaire absent des créneaux restants remonte nettement ;
      // un morceau déjà très présent est au contraire freiné.
      final futureCount = futureCountFor(p.id);
      final futureDays = futureDaysFor(p.id).length;
      if (futureCount == 0) score += p.priority ? 42 : 18;
      else if (futureCount == 1 && p.priority) score += 12;
      if (futureDays == 0 && p.priority) score += 8;
      if (weeklyOverconcentration(p.id) && !p.priority) score -= 28;
      if (futureCount >= 3 && !p.priority) score -= 18;
    ...
    String? nextFocus(Project p, Session s) {
      ...
      if (s.coachFeeling == 'difficile' || _recentDifficultSessions(p.id) >= 2) {
        if (current != 'Consolidation') return 'Consolidation';
        return p.weakestStageName;
      }
    ...
    int? meaningfulDuration(int current, String focus, bool increase) {
      ...
      // Une adaptation du coach doit représenter un vrai bloc de travail :
      // aucun changement de 1 à 9 minutes n'est autorisé.
      ...
      if (increase) {
        ...
        return bounded - current >= 10 ? bounded : null;
      }
      ...
      return current - candidate >= 10 && candidate >= min ? candidate : null;
    }

    bool applyDuration(PlanItem item, int newDuration, String reason) {
      // V265 — garde-fou final : une décision du coach ne peut viser qu'un
      ...
      final durationFocus = _focusFor(projectById(item.projectId) ?? candidateProject);
      ...
      if (lastAt != null && previous != null) {
        ...
        // Jamais de ping-pong rapide (30→20→30) sur le même créneau.
        if (age < const Duration(hours: 72)) {
          if (lastDirection * newDirection < 0) return false;
          if (item.coachAdjustmentType == 'duration') return false;
        }
      }
      ...
      item.coachAdjustedAt = now;
      item.coachPreviousDuration = old;
      item.coachAdjustmentType = 'duration';
      item.coachPreviousFocus = null;
      item.coachNewFocus = null;
      _lastCoachChangedItemId = item.id;
      _lastCoachOldDuration = old;
      _lastCoachNewDuration = newDuration;
      _lastCoachReason = reason;
      _lastCoachAdjustmentType = 'duration';
      return true;
    }
    ...
    bool applyFocus(PlanItem item, Project p, String newFocus, String reason, {bool strongSignal = false}) {
      ...
      _lastCoachOldFocus = oldFocus;
      _lastCoachNewFocus = newFocus;
      return true;
    }
    ...
    final candidateEvidence = plannedEvidenceCount(candidateProject.id);
    final candidateFutureMinutes = futureMinutesFor(candidateProject.id);
    final candidateFutureCount = futureCountFor(candidateProject.id);
    final candidateFutureDays = futureDaysFor(candidateProject.id).length;
    final candidateOverconcentrated = weeklyOverconcentration(candidateProject.id);
    ...
    if (candidateProject.id == latestProject.id &&
        (latestSession.coachFeeling == 'difficile' ||
            ...
            (candidateEvidence >= 2 && candidateAdherence < .78))) {
    ...
    final increaseEvidenceOk =
        candidateEvidence >= 2 && candidateAdherence >= 1.08;
    final freshPriorityNeed =
        candidateProject.priority && candidateEvidence == 0 && candidateDays >= 4;
    ...
    // 3b) Un morceau très prometteur mais avec une seule mesure d'adhérence ne
    //     déclenche pas encore une augmentation automatique : le coach attend
    //     une seconde séance comparable plutôt que de surinterpréter un seul essai.
    if (candidateProject.id != latestProject.id &&
        candidateProject.priority &&
        candidateEvidence > 0 &&
        candidateEvidence < 2 &&
        candidateAdherence >= 1.0) {
      _lastCoachReason =
          'J’ai réexaminé ${candidateProject.name}. Le morceau reste prioritaire, mais je n’augmente pas encore sa durée : il n’y a pas assez de séances comparables pour justifier un changement automatique.';
      return false;
    }
    ...
    if (candidateProject.id != latestProject.id && candidateOverconcentrated) {
      _lastCoachReason =
  ...
  void _auditPlanningIntegrity() {
    ...
    final sessionById = <String, Session>{for (final s in sessions) s.id: s};
    plan.removeWhere((item) {
      if (!seenIds.add(item.id)) return true;

      // 0 minute peut être une valeur volontaire pour une ligne de repos / repère.
      // On ne transforme donc plus automatiquement 0 en 1 minute.
      if (item.duration < 0) item.duration = 0;
      // plannedDuration est final : les sauvegardes anciennes avec une valeur négative
      // sont normalisées dans PlanItem.fromJson().

      final sourceId = item.sourceSessionId;
      if (sourceId != null) {
        final source = sessionById[sourceId];
        if (source == null) {
          // Le lien est devenu orphelin (ancienne sauvegarde ou suppression).
          // On conserve la ligne du planning mais on retire uniquement le lien technique.
          item.sourceSessionId = null;
        } else {
          item.completed = true;
          if (item.projectId == null && source.projectId != null) {
            item.projectId = source.projectId;
          }
          if (item.title.trim().isEmpty && source.projectId != null) {
            item.title = projectById(source.projectId)?.name ?? item.title;
          }
        }
      }
  ...
  void _auditSessionIntegrity() {
    ...
    // Une décision coach active ne doit pas rester l'analyse courante si sa session
    // source n'existe plus, notamment après restauration d'une ancienne sauvegarde.
    final existingSessionIds = sessions.map((s) => s.id).toSet();
    final now = DateTime.now();
    for (final d in coachDecisionLog) {
      if (d.isActive && d.sessionId != null && !existingSessionIds.contains(d.sessionId)) {
        d.status = 'removed';
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        scaffoldBackgroundColor: phone ? const Color(0xFFF3F4F8) : const Color(0xFFF6F7FB),
        dividerTheme: const DividerThemeData(space: 14, thickness: 1, indent: 0, endIndent: 0),
        listTileTheme: ListTileThemeData(
          contentPadding: EdgeInsets.symmetric(horizontal: phone ? 4 : 2, vertical: phone ? 2 : 1),
          minLeadingWidth: 32,
          minVerticalPadding: phone ? 4 : 2,
        ),
        iconButtonTheme: IconButtonThemeData(
          style: ButtonStyle(
            minimumSize: WidgetStatePropertyAll(Size(phone ? 44 : 40, phone ? 44 : 40)),
            padding: WidgetStatePropertyAll(EdgeInsets.all(phone ? 9 : 8)),
            tapTargetSize: MaterialTapTargetSize.padded,
          ),
        ),
        dialogTheme: DialogThemeData(
          insetPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 20),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24), side: BorderSide(color: phone ? Colors.black12 : Colors.transparent, width: phone ? 1 : 0)),
          titleTextStyle: TextStyle(fontSize: 20, fontWeight: FontWeight.w800),
        ),
        appBarTheme: AppBarTheme(
          elevation: 0,
          scrolledUnderElevation: 0,
          toolbarHeight: phone ? (compactPhone ? 50 : 52) : 64,
          centerTitle: false,
          titleSpacing: phone ? 12 : 16,
          iconTheme: IconThemeData(size: phone ? 22 : 24),
          actionsIconTheme: IconThemeData(size: phone ? 22 : 24),
          titleTextStyle: TextStyle(fontSize: phone ? (compactPhone ? 18 : 19) : 20, fontWeight: FontWeight.w800, letterSpacing: -.2),
        ),
        cardTheme: CardThemeData(
          ...
          shape: RoundedRectangleBorder(
            ...
            side: BorderSide(color: phone ? Colors.black12 : Colors.transparent, width: phone ? 1 : 0),
          ),
        ),
        inputDecorationTheme: InputDecorationTheme(
          ...
          contentPadding: EdgeInsets.symmetric(horizontal: phone ? 14 : 13, vertical: phone ? 13 : 12),
        ),
        filledButtonTheme: FilledButtonThemeData(
          style: FilledButton.styleFrom(
            minimumSize: Size(0, phone ? 48 : 46),
        ...
        outlinedButtonTheme: OutlinedButtonThemeData(
          style: OutlinedButton.styleFrom(
            minimumSize: Size(0, phone ? 46 : 44),
            padding: EdgeInsets.symmetric(horizontal: 16, vertical: phone ? 12 : 11),
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
          ),
        ),
        navigationBarTheme: NavigationBarThemeData(
          height: phone ? (compactPhone ? 70 : 72) : 68,
          elevation: 0,
          backgroundColor: const Color(0xFFF9FAFD),
          indicatorShape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(phone ? 18 : 22)),
          labelBehavior: NavigationDestinationLabelBehavior.alwaysShow,
          indicatorColor: phone ? const Color(0xFFE1DDFF) : const Color(0xFFE6E2FF),
          iconTheme: WidgetStateProperty.resolveWith<IconThemeData?>((states) {
            final selected = states.contains(WidgetState.selected);
            return IconThemeData(size: selected ? (phone ? 23 : 24) : (phone ? 22 : 24), weight: selected ? 700 : null);
          }),
          labelTextStyle: WidgetStateProperty.resolveWith<TextStyle?>((states) {
            final selected = states.contains(WidgetState.selected);
            return TextStyle(fontSize: phone ? (compactPhone ? 10 : 10.5) : 10.5, fontWeight: selected ? FontWeight.w800 : FontWeight.w600);
          }),
        ),
      ),
      darkTheme: ThemeData(
        ...
        visualDensity: VisualDensity.standard,
        scaffoldBackgroundColor: phone ? const Color(0xFF131318) : null,
        dividerTheme: const DividerThemeData(space: 14, thickness: 1, indent: 0, endIndent: 0),
        listTileTheme: ListTileThemeData(
          contentPadding: EdgeInsets.symmetric(horizontal: phone ? 4 : 2, vertical: phone ? 2 : 1),
          minLeadingWidth: 32,
        ...
        dialogTheme: DialogThemeData(
          insetPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 20),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24), side: BorderSide(color: phone ? Colors.white12 : Colors.transparent, width: phone ? 1 : 0)),
          titleTextStyle: TextStyle(fontSize: 20, fontWeight: FontWeight.w800),
        ),
        appBarTheme: AppBarTheme(
          elevation: 0,
        ...
        cardTheme: CardThemeData(
          ...
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(18),
            side: BorderSide(color: phone ? Colors.white12 : Colors.transparent, width: phone ? 1 : 0),
          ),
        ),
        inputDecorationTheme: InputDecorationTheme(
          filled: true,
        ...
        navigationBarTheme: NavigationBarThemeData(
          ...
          indicatorShape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(phone ? 18 : 22)),
          labelBehavior: NavigationDestinationLabelBehavior.alwaysShow,
          indicatorColor: phone ? const Color(0xFF45406E) : const Color(0xFF3E3966),
          iconTheme: WidgetStateProperty.resolveWith<IconThemeData?>((states) {
            final selected = states.contains(WidgetState.selected);
            return IconThemeData(size: selected ? (phone ? 23 : 24) : (phone ? 22 : 24), weight: selected ? 700 : null);
          }),
      ...
      home: Scaffold(
        ...
        bottomNavigationBar: Container(
          decoration: BoxDecoration(
            ...
            border: Border(top: BorderSide(color: darkMode ? Colors.white10 : (phone ? const Color(0x1A000000) : const Color(0x14000000)), width: phone ? 0.8 : 1)),
          ),
          child: NavigationBar(
            selectedIndex: tab,
...
class CardBox extends StatelessWidget {
  const CardBox({super.key, required this.child, this.padding = const EdgeInsets.all(18), this.accentColor});
  final Widget child;
  ...
  Widget build(BuildContext c) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: phone
            ? (accentColor == null
                ? Border.all(color: scheme.outlineVariant.withOpacity(.52), width: .7)
                : Border(
                    left: BorderSide(color: accentColor!, width: 3),
                    top: BorderSide(color: scheme.outlineVariant.withOpacity(.46), width: .7),
                    right: BorderSide(color: scheme.outlineVariant.withOpacity(.46), width: .7),
                    bottom: BorderSide(color: scheme.outlineVariant.withOpacity(.46), width: .7),
                  ))
            : Border.all(color: scheme.outlineVariant.withOpacity(.38)),
      ),
...
class WeeklyPlanComparison {
  WeeklyPlanComparison({
    required this.planItemId,
    required this.date,
    required this.projectName,
    required this.plannedMinutes,
    required this.realizedMinutes,
    required this.completed,
    required this.coachAdjusted,
    required this.coachAdjustmentType,
  });
  final String planItemId;
  final DateTime date;
  final String projectName;
  final int plannedMinutes;
  final int? realizedMinutes;
  final bool completed;
  final bool coachAdjusted;
  final String? coachAdjustmentType;

  double? get adherence {
    if (!completed || realizedMinutes == null || plannedMinutes <= 0) return null;
...
class WeeklyBilan {
  WeeklyBilan({
    ...
    required this.plannedMinutes,
    required this.duePlannedMinutes,
    required this.upcomingPlannedMinutes,
    required this.realizedPlannedMinutes,
    required this.plannedSessions,
    required this.duePlannedSessions,
    required this.upcomingPlannedSessions,
    required this.completedPlannedSessions,
    required this.planComparisons,
  ...
  final int plannedMinutes;
  final int duePlannedMinutes;
  final int upcomingPlannedMinutes;
  final int realizedPlannedMinutes;
  final int plannedSessions;
  final int duePlannedSessions;
  final int upcomingPlannedSessions;
  final int completedPlannedSessions;
  final List<WeeklyPlanComparison> planComparisons;
            ...
            Row(children: [
              ...
              Expanded(child: Row(children: [
                ...
                if (next != null) Container(
                  ...
                  decoration: BoxDecoration(
                    color: scheme.onPrimaryContainer.withOpacity(.10),
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: Text('▶ PROCHAINE', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w900, letterSpacing: .3, color: scheme.onPrimaryContainer)),
                ),
              ])),
        ...
        Container(
          ...
          child: Column(children: [
            ...todayItems.take(5).expand((item) => [
              ...
              if (item != todayItems.take(5).last) Divider(height: 1, indent: 56, endIndent: 8),
            ]),
        ...
        CardBox(padding: const EdgeInsets.all(14), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(child: latestCoachDecision == null
              ...
              : Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  if ((latestCoachDecision!.reason ?? '').trim().isNotEmpty) ...[const SizedBox(height: 4), Text('Pourquoi : ${_coachShortReason(latestCoachDecision!.reason)}', softWrap: true, style: TextStyle(fontSize: 11.5, height: 1.35, color: scheme.onSurfaceVariant))],
                ])),
        ])),
...
class _SessionsState extends State<Sessions> {
  ...
  String _plannedRealizedLabel(Session s) {
    final planned = s.plannedDuration;
    if (planned == null || planned <= 0) return '';
    final percent = (s.duration / planned * 100).round();
    if (percent == 100) return 'Plan respecté';
    return '${s.duration} / ${planned} min · $percent %';
  }

  String _runThroughLabel(Session s) {
    if (s.type != 'Run-through' || s.runThroughCompleted == null) return '';
  ...
  Widget build(BuildContext c) {
    final phone = MediaQuery.sizeOf(c).width < 600;
    final compact = MediaQuery.sizeOf(c).width < 430;
    final list = filtered;
    ...
    final totalMinutes = list.fold<int>(0, (a, s) => a + s.duration);
    final plannedSessions = list.where((s) => s.plannedDuration != null && s.plannedDuration! > 0).toList();
    final plannedTotal = plannedSessions.fold<int>(0, (a, s) => a + (s.plannedDuration ?? 0));
    final realizedTotalForPlanned = plannedSessions.fold<int>(0, (a, s) => a + s.duration);
    final realizationRate = plannedTotal > 0 ? (realizedTotalForPlanned / plannedTotal * 100).round() : null;
    final hasFilters = search.isNotEmpty || typeFilter != null || methodFilter != null || periodDays != 0;

    Widget stat(String label, String value, IconData icon) => Expanded(child: Container(
      padding: EdgeInsets.all(phone ? 12 : 14),
      decoration: BoxDecoration(
        color: Theme.of(c).colorScheme.surfaceContainerHighest.withOpacity(.48),
      ...
      child: Row(children: [
        ...
        const SizedBox(width: 8),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: TextStyle(fontSize: 10.5, color: Theme.of(c).colorScheme.onSurfaceVariant)),
          const SizedBox(height: 2),
    ...
    return Column(children: [
      AppBar(
        ...
        actions: [
          if (phone)
            IconButton(tooltip: 'Nouvelle session', onPressed: widget.onAdd, icon: const Icon(Icons.add_rounded))
          else
            FilledButton.icon(onPressed: widget.onAdd, icon: const Icon(Icons.add_rounded), label: const Text('Nouvelle')),
        ],
      ),
      Expanded(child: ListView(padding: EdgeInsets.fromLTRB(phone ? 12 : 16, 10, phone ? 12 : 16, 22), children: [
        Row(children: [
          stat('Sessions', '${list.length}', Icons.music_note_outlined),
          const SizedBox(width: 8),
          stat('Temps', _mins(totalMinutes), Icons.timer_outlined),
          const SizedBox(width: 8),
          stat('Série', '${_currentStreak(byDay)} j', Icons.local_fire_department_outlined),
          if (!phone && realizationRate != null) ...[
            const SizedBox(width: 8),
            stat('Prévu → réel', '$realizationRate %', Icons.track_changes_outlined),
          ],
        ]),
        if (phone && realizationRate != null) ...[
          const SizedBox(height: 7),
          Align(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: Theme.of(c).colorScheme.primaryContainer.withOpacity(.58),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: Theme.of(c).colorScheme.primary.withOpacity(.16)),
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Icon(Icons.track_changes_outlined, size: 15, color: Theme.of(c).colorScheme.primary),
                  const SizedBox(width: 6),
                  Text('Prévu → réel : $realizationRate %', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Theme.of(c).colorScheme.primary)),
                ],
              ),
            ),
          ),
        ...
        if (!phone) ...[
          ...
          CardBox(padding: const EdgeInsets.fromLTRB(12, 10, 12, 11), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: _sectionHeader(c, Icons.bar_chart_outlined, '7 derniers jours')),
              Text(_mins(List.generate(7, (i) => byDay[_day(DateTime.now()).subtract(Duration(days: 6 - i))] ?? 0).fold<int>(0, (a, b) => a + b)), style: TextStyle(fontWeight: FontWeight.w800, color: Theme.of(c).colorScheme.primary, fontSize: 12)),
            ]),
            const SizedBox(height: 8),
            SizedBox(height: compact ? 82 : 96, child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: List.generate(7, (i) {
              final d = _day(DateTime.now()).subtract(Duration(days: 6 - i));
              final value = byDay[d] ?? 0;
              final maxValue = byDay.values.fold<int>(0, (a, b) => a > b ? a : b);
              final h = maxValue == 0 ? 2.0 : (56 * value / maxValue).clamp(2.0, 56.0);
              return Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 2), child: Column(mainAxisAlignment: MainAxisAlignment.end, children: [
                if (value > 0) Text('$value', style: const TextStyle(fontSize: 9.5, fontWeight: FontWeight.w700)),
                const SizedBox(height: 2),
                Container(height: h, decoration: BoxDecoration(color: Theme.of(c).colorScheme.primaryContainer, borderRadius: BorderRadius.circular(4))),
                const SizedBox(height: 4),
        ...
        CardBox(padding: EdgeInsets.all(phone ? 13 : 18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Expanded(child: _sectionHeader(c, Icons.filter_alt_outlined, 'Filtrer / rechercher')),
            if (hasFilters) IconButton(tooltip: 'Réinitialiser', visualDensity: VisualDensity.compact, onPressed: () => setState(() { search=''; typeFilter=null; methodFilter=null; periodDays=0; sort='recent'; }), icon: const Icon(Icons.refresh_rounded, size: 19)),
          ]),
          const SizedBox(height: 8),
          TextField(
            decoration: InputDecoration(labelText: 'Rechercher', prefixIcon: const Icon(Icons.search), isDense: true, suffixIcon: search.isEmpty ? null : IconButton(onPressed: () => setState(() => search=''), icon: const Icon(Icons.clear_rounded, size: 18))),
            onChanged: (v) => setState(() => search = v),
          ),
          const SizedBox(height: 9),
          SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: [
            PopupMenuButton<String>(
              initialValue: typeFilter ?? '__all__',
              onSelected: (v) => setState(() => typeFilter = v == '__all__' ? null : v),
              itemBuilder: (_) => [const PopupMenuItem<String>(value: '__all__', child: Text('Tous les types')), const PopupMenuItem<String>(value: 'Run-through', child: Text('Run-through')), ...objectiveCategories.map((v) => PopupMenuItem<String>(value: v, child: Text(v)))],
              child: _mobileFilterButton(c, Icons.category_outlined, typeFilter ?? 'Type', active: typeFilter != null),
            ),
            const SizedBox(width: 7),
            PopupMenuButton<String>(
              initialValue: methodFilter ?? '__all__',
              onSelected: (v) => setState(() => methodFilter = v == '__all__' ? null : v),
              itemBuilder: (_) => [const PopupMenuItem<String>(value: '__all__', child: Text('Toutes les méthodes')), ...methods.map((v) => PopupMenuItem<String>(value: v, child: Text(v)))],
              child: _mobileFilterButton(c, Icons.smartphone_outlined, methodFilter ?? 'Méthode', active: methodFilter != null),
            ),
            const SizedBox(width: 7),
            PopupMenuButton<int>(
              initialValue: periodDays,
              onSelected: (v) => setState(() => periodDays = v),
              itemBuilder: (_) => const [PopupMenuItem(value: 0, child: Text('Toute la période')), PopupMenuItem(value: 7, child: Text('7 derniers jours')), PopupMenuItem(value: 30, child: Text('30 derniers jours'))],
              child: _mobileFilterButton(c, Icons.date_range_outlined, periodDays == 0 ? 'Période' : '${periodDays} j', active: periodDays != 0),
            ),
            const SizedBox(width: 7),
            PopupMenuButton<String>(
              initialValue: sort,
              onSelected: (v) => setState(() => sort = v),
              itemBuilder: (_) => const [PopupMenuItem(value: 'recent', child: Text('Plus récentes')), PopupMenuItem(value: 'long', child: Text('Durée longue')), PopupMenuItem(value: 'short', child: Text('Durée courte')), PopupMenuItem(value: 'piece', child: Text('Par morceau'))],
              child: _mobileFilterButton(c, Icons.swap_vert_rounded, sort == 'recent' ? 'Récent' : sort == 'long' ? 'Longue' : sort == 'short' ? 'Courte' : 'Morceau'),
            ),
          ])),
          const SizedBox(height: 7),
          Text('${list.length} session${list.length > 1 ? 's' : ''} · ${_mins(totalMinutes)}', style: TextStyle(fontSize: 11.5, color: Theme.of(c).colorScheme.onSurfaceVariant)),
        ])),
        const SizedBox(height: 12),
        if (list.isEmpty)
          emptyState('Aucune session ne correspond aux critères.')
        else
          ...list.map((s) {
            final project = widget.projectById(s.projectId);
            final feeling = _feelingLabel(s.coachFeeling);
            return Padding(
              padding: const EdgeInsets.only(bottom: 8),
              child: swipeToDelete(
                key: ValueKey(s.id),
                ...
                child: CardBox(
                  padding: EdgeInsets.fromLTRB(phone ? 12 : 16, phone ? 12 : 15, phone ? 10 : 14, phone ? 12 : 15),
                  accentColor: phone ? categoryColor(s.type) : null,
                  child: InkWell(
                    borderRadius: BorderRadius.circular(phone ? 15 : 17),
                    onTap: () => widget.onEdit(s),
                    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Container(width: phone ? 40 : 42, height: phone ? 40 : 42, alignment: Alignment.center,
                        decoration: BoxDecoration(color: categoryColor(s.type).withOpacity(.14), borderRadius: BorderRadius.circular(12)),
                        child: Icon(categoryIcon(s.type), size: phone ? 19 : 19, color: categoryColor(s.type))),
                      const SizedBox(width: 10),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Expanded(child: Text(project?.name ?? 'Pratique libre', maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: phone ? 15.5 : 15.5, fontWeight: FontWeight.w800, height: 1.15))),
                          const SizedBox(width: 7),
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                            decoration: BoxDecoration(color: Theme.of(c).colorScheme.surfaceContainerHighest.withOpacity(.70), borderRadius: BorderRadius.circular(12)),
                            child: Text('${s.duration} min', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w800, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                          ),
                        ]),
                        const SizedBox(height: 3),
                        Wrap(
                          ...
                          children: [
                            Text('${s.date.day.toString().padLeft(2,'0')}/${s.date.month.toString().padLeft(2,'0')}/${s.date.year}${s.rating > 0 ? ' · ' + ('★' * s.rating) : ''}', style: TextStyle(fontSize: 11, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                            if (_isToday(s.date))
                              Container(
                                padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3),
                                decoration: BoxDecoration(
                                  color: Theme.of(c).colorScheme.primaryContainer.withOpacity(.72),
                                  borderRadius: BorderRadius.circular(9),
                                  border: Border.all(color: Theme.of(c).colorScheme.primary.withOpacity(.22)),
                                ),
                        ...
                        Wrap(spacing: 5, runSpacing: 5, children: [
                          _sessionMetaPill(c, Icons.category_outlined, s.type),
                          if (s.coachFocus != null && s.coachFocus!.isNotEmpty) _sessionMetaPill(c, Icons.center_focus_strong_outlined, s.coachFocus!),
                          if (!compact && feeling.isNotEmpty) _sessionMetaPill(c, Icons.psychology_outlined, feeling),
                          if (s.plannedDuration != null && s.plannedDuration! > 0) _sessionMetaPill(c, Icons.schedule_outlined, _plannedRealizedLabel(s)),
                          if (s.type == 'Run-through' && s.runThroughCompleted != null) _sessionMetaPill(c, s.runThroughCompleted! ? Icons.check_circle_outline : Icons.stop_circle_outlined, _runThroughLabel(s)),
                        ]),
                        if (phone && (_plannedRealizedLabel(s).isNotEmpty || _runThroughLabel(s).isNotEmpty)) ...[
                          const SizedBox(height: 6),
                          Row(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Icon(Icons.insights_outlined, size: 14, color: Theme.of(c).colorScheme.primary),
                              const SizedBox(width: 5),
                              Expanded(
                                child: Text(
                                  [
                                    if (_plannedRealizedLabel(s).isNotEmpty) _plannedRealizedLabel(s),
                                    if (_runThroughLabel(s).isNotEmpty) _runThroughLabel(s),
                                  ].join(' · '),
                                  style: TextStyle(fontSize: 10.8, fontWeight: FontWeight.w700, color: Theme.of(c).colorScheme.onSurfaceVariant),
                                ),
                              ),
                            ],
                          ),
                        ],
                        if (!compact && s.coachRecommendation != null && s.coachRecommendation!.trim().isNotEmpty) ...[
                          const SizedBox(height: 6),
                          Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Icon(Icons.auto_awesome_rounded, size: 14, color: Theme.of(c).colorScheme.primary),
                            const SizedBox(width: 5),
                            Expanded(child: Text(s.coachRecommendation!, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, height: 1.3, color: Theme.of(c).colorScheme.onSurfaceVariant))),
                          ]),
                        ],
                      ])),
                      PopupMenuButton<String>(
                        ...
                        onSelected: (action) async {
                          if (action == 'details') await _showSessionDetails(c, s);
                          if (action == 'edit') widget.onEdit(s);
                          if (action == 'delete' && await confirmDelete(c, 'cette session')) widget.onDelete(s);
                        },
                        itemBuilder: (_) => const [
                          PopupMenuItem(value: 'details', child: Text('Voir le bilan')),
...
class _SessionDialogState extends State<SessionDialog> {
  ...
  Widget build(BuildContext c) {
    ...
    return Theme(
      data: flatTheme,
      child: AlertDialog(
        insetPadding: EdgeInsets.symmetric(horizontal: phone ? 8 : 12, vertical: phone ? 10 : 18),
        titlePadding: EdgeInsets.fromLTRB(phone ? 18 : 22, phone ? 16 : 18, phone ? 18 : 22, 6),
        contentPadding: EdgeInsets.fromLTRB(phone ? 18 : 22, 4, phone ? 18 : 22, 4),
        actionsPadding: EdgeInsets.fromLTRB(phone ? 12 : 18, phone ? 7 : 2, phone ? 12 : 18, phone ? 12 : 12),
...
class _ProjectsState extends State<Projects> {
  ...
  Widget build(BuildContext c) {
    ...
    return Column(children: [
      AppBar(
        title: Text('Morceaux · ${widget.items.length}'),
        actions: [
          if (phone)
            IconButton(tooltip: 'Nouveau morceau', onPressed: widget.onAdd, icon: const Icon(Icons.add_rounded))
          else
            FilledButton.icon(onPressed: widget.onAdd, icon: const Icon(Icons.add_rounded), label: const Text('Nouveau')),
        ],
      ),
      Padding(
        padding: EdgeInsets.fromLTRB(phone ? 12 : 16, phone ? 8 : 10, phone ? 12 : 16, 0),
        child: Row(children: [
          Expanded(
            child: PopupMenuButton<String>(
              initialValue: filterStatus ?? '__all__',
              onSelected: (v) => setState(() => filterStatus = v == '__all__' ? null : v),
              itemBuilder: (_) => [
                const PopupMenuItem<String>(value: '__all__', child: Text('Tous les morceaux')),
                ...projectStatuses.map((status) => PopupMenuItem<String>(value: status, child: Text('${projectStatusEmoji(status)}  $status'))),
              ],
              child: _mobileFilterButton(c, Icons.filter_list_rounded, filterStatus == null ? 'Tous' : filterStatus!, active: filterStatus != null),
            ),
          ),
          const SizedBox(width: 8),
          Expanded(
            child: PopupMenuButton<String>(
              ...
              itemBuilder: (_) => const [
                PopupMenuItem(value: 'recent', child: Text('Dernière modification')),
                PopupMenuItem(value: 'status', child: Text('Statut')),
                PopupMenuItem(value: 'name', child: Text('Nom')),
              ],
              child: _mobileFilterButton(c, Icons.swap_vert_rounded, sortMode == 'recent' ? 'Récent' : sortMode == 'status' ? 'Statut' : 'Nom'),
            ),
          ),
          const SizedBox(width: 8),
          _countPill(c, '${sorted.length}'),
        ]),
      ),
      Expanded(
        child: widget.items.isEmpty
            ...
            : sorted.isEmpty
                ? emptyState('Aucun morceau ne correspond au filtre.')
                : ListView.separated(
                    padding: EdgeInsets.fromLTRB(phone ? 12 : 16, 12, phone ? 12 : 16, 20),
                    itemCount: sorted.length,
                    separatorBuilder: (_, __) => SizedBox(height: phone ? 11 : 10),
                    itemBuilder: (_, i) {
                      final p = sorted[i];
                      ...
                      final pieceRunMastery = (completedPieceRuns.isNotEmpty || p.currentTempo > 0)
                          ...
                          : null;
                      final lastModified = _lastModifiedFor(p);
                      final statusColor = _statusStrongColor(c, p.effectiveStatus);
                      return swipeToDelete(
                        key: ValueKey(p.id),
                        what: 'ce morceau',
                        onDelete: () => widget.onDelete(p),
                        child: CardBox(
                          padding: EdgeInsets.all(phone ? (compact ? 14 : 16) : 18),
                          accentColor: phone ? statusColor : null,
                          child: InkWell(
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                Container(
                                  ...
                                  alignment: Alignment.center,
                                  decoration: BoxDecoration(
                                    color: Theme.of(c).colorScheme.primaryContainer.withOpacity(.60),
                                    borderRadius: BorderRadius.circular(14),
                                  ),
                                  child: Text(p.emoji, style: TextStyle(fontSize: compact ? 23 : 25)),
                                ),
                                const SizedBox(width: 11),
                                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    Expanded(child: Text(p.name, maxLines: 2, overflow: TextOverflow.ellipsis,
                                      style: TextStyle(fontSize: phone ? 16 : 18, fontWeight: FontWeight.w800, height: 1.15))),
                                    if (p.priority) ...[
                                      const SizedBox(width: 5),
                                  ...
                                  const SizedBox(height: 4),
                                  Text(
                                    '${projectStatusEmoji(p.effectiveStatus)} ${p.effectiveStatus} · ${_formatProjectDateTime(lastModified)}',
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                                    style: TextStyle(fontSize: 11.5, color: Theme.of(c).colorScheme.onSurfaceVariant),
                                  ),
                                ])),
                                PopupMenuButton<String>(
                                  tooltip: 'Actions',
                                  icon: const Icon(Icons.more_horiz_rounded),
                                  onSelected: (action) async {
                                    if (action == 'open') widget.onEdit(p);
                                    if (action == 'duplicate') widget.onDuplicate(p);
                                    if (action == 'delete' && await confirmDelete(c, 'ce morceau')) widget.onDelete(p);
                                  },
                                  itemBuilder: (_) => const [
                                    PopupMenuItem(value: 'open', child: Text('Ouvrir la fiche')),
                              ...
                              const SizedBox(height: 12),
                              Row(children: [
                                Text('Progression', style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w700, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                                const SizedBox(width: 8),
                                ...
                                const SizedBox(width: 10),
                                Text('${(p.progress * 100).round()} %', style: TextStyle(fontSize: phone ? 16 : 17, fontWeight: FontWeight.w900, color: Theme.of(c).colorScheme.primary)),
                              ]),
                              const SizedBox(height: 8),
                              Wrap(
                                spacing: 7,
                                ...
                                children: [
                                  Container(
                                    ...
                                    decoration: BoxDecoration(color: _statusSoftColor(c, p.effectiveStatus), borderRadius: BorderRadius.circular(18)),
                                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                                      Text(projectStatusEmoji(p.effectiveStatus), style: TextStyle(color: statusColor, fontSize: 12)),
                                      const SizedBox(width: 5),
                                      Text(p.effectiveStatus, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: statusColor, fontSize: 10.5, fontWeight: FontWeight.w800)),
                                    ]),
                                  ),
                                  if (p.currentTempo > 0 || p.targetTempo > 0)
                                    _sessionMetaPill(c, Icons.speed_outlined, p.currentTempo > 0 && p.targetTempo > 0 ? '${p.currentTempo} → ${p.targetTempo} BPM' : '${p.currentTempo > 0 ? p.currentTempo : p.targetTempo} BPM'),
                                  if (pieceRunMastery != null)
                                    _sessionMetaPill(c, Icons.play_circle_outline, 'Run-through ${pieceRunMastery.round()} %'),
                                ],
                              ),
                              if (p.goal.trim().isNotEmpty) ...[
                                ...
                                Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  Container(
                                    ...
                                    alignment: Alignment.center,
                                    decoration: BoxDecoration(
                                      color: Theme.of(c).colorScheme.primaryContainer.withOpacity(.55),
                                      borderRadius: BorderRadius.circular(9),
                                    ),
                                    child: Icon(Icons.flag_outlined, size: 15, color: Theme.of(c).colorScheme.primary),
                                  ),
                                  const SizedBox(width: 8),
                                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    Text('Objectif', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w800, color: Theme.of(c).colorScheme.primary)),
                                    const SizedBox(height: 2),
                                    Text(p.goal, maxLines: phone ? 2 : 3, overflow: TextOverflow.ellipsis,
                                      style: TextStyle(fontSize: 12.5, color: Theme.of(c).colorScheme.onSurfaceVariant, height: 1.3)),
                                  ])),
                                ]),
                              ],
                              const SizedBox(height: 10),
                              Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                                Icon(Icons.track_changes_outlined, size: 15, color: Theme.of(c).colorScheme.primary),
                                const SizedBox(width: 6),
                                Text('À renforcer', style: TextStyle(fontSize: 11.5, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                                const SizedBox(width: 7),
                                Flexible(child: Container(
                                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 5),
                                  decoration: BoxDecoration(
                                    color: Theme.of(c).colorScheme.primaryContainer.withOpacity(.62),
                                    borderRadius: BorderRadius.circular(12),
                                  ),
                                  child: Text(_weakestStepLabel(p), maxLines: 1, overflow: TextOverflow.ellipsis,
                                    style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w800, color: Theme.of(c).colorScheme.onPrimaryContainer)),
                                )),
                                if (!compact && (p.method != null || p.targetHours != null)) ...[
                                  const SizedBox(width: 8),
                                  Flexible(child: Text(
                                    p.method ?? 'Auto · ${p.targetHours!.round()} h',
                                    maxLines: 1,
                                    overflow: TextOverflow.ellipsis,
                              ...
                              if (phone) ...[
                                ...
                                Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                                  Text('Ouvrir la fiche', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: Theme.of(c).colorScheme.primary)),
                                  const SizedBox(width: 3),
                                  Icon(Icons.chevron_right_rounded, size: 17, color: Theme.of(c).colorScheme.primary),
                                ]),
                              ],
                            ]),
                          ),
...
Widget _mobileFilterButton(BuildContext c, IconData icon, String label, {bool active = false}) {
  ...
  return Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 6),
      Text(label, style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w700, color: active ? scheme.primary : scheme.onSurface)),
      const SizedBox(width: 3),
...
Widget _countPill(BuildContext c, String value) {
  final scheme = Theme.of(c).colorScheme;
  return Container(
    ...
    child: Text(value, style: TextStyle(color: scheme.onPrimary, fontSize: 12, fontWeight: FontWeight.w900)),
  );
}
...
Widget _projectStageMini(BuildContext c, String label, double value, IconData icon) {
  final scheme = Theme.of(c).colorScheme;
  final pct = (value * 100).round();
  return SizedBox(
    width: _isPhoneLayout(c) ? 128 : 150,
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        Text('$pct%', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w900, color: scheme.primary)),
      ]),
      const SizedBox(height: 4),
      ClipRRect(
        borderRadius: BorderRadius.circular(5),
        child: LinearProgressIndicator(value: value.clamp(0.0, 1.0), minHeight: 5),
      ),
    ]),
  );
}

Widget _projectEditSummary(BuildContext c, Project p) {
  final scheme = Theme.of(c).colorScheme;
  final phone = _isPhoneLayout(c);
  final progressPct = (p.progress * 100).round();
  final tempo = p.currentTempo > 0 ? '${p.currentTempo} BPM' : 'Tempo —';
  final target = p.targetTempo > 0 ? 'cible ${p.targetTempo}' : 'cible —';
  return Container(
    width: double.infinity,
    padding: EdgeInsets.all(phone ? 13 : 12),
    decoration: BoxDecoration(
      color: scheme.surfaceContainerHighest.withOpacity(.50),
      borderRadius: BorderRadius.circular(phone ? 16 : 13),
      border: Border.all(color: scheme.outlineVariant.withOpacity(.72)),
    ),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
        Container(
          ...
          decoration: BoxDecoration(
            color: scheme.primaryContainer.withOpacity(.72),
            borderRadius: BorderRadius.circular(phone ? 12 : 10),
          ),
          child: Text(p.emoji, style: TextStyle(fontSize: phone ? 23 : 21)),
        ),
        const SizedBox(width: 10),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(p.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: phone ? 15.5 : 15, fontWeight: FontWeight.w900, height: 1.15)),
          const SizedBox(height: 4),
          Wrap(spacing: 6, runSpacing: 4, children: [
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
              decoration: BoxDecoration(color: scheme.primaryContainer.withOpacity(.62), borderRadius: BorderRadius.circular(12)),
              child: Text(p.effectiveStatus, style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w800, color: scheme.onPrimaryContainer)),
            ),
            if (p.priority)
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                decoration: BoxDecoration(color: Colors.amber.withOpacity(.18), borderRadius: BorderRadius.circular(12)),
                child: const Text('★ Priorité', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w800)),
              ),
          ]),
        ])),
      ]),
      const SizedBox(height: 12),
      Row(children: [
        Expanded(child: ClipRRect(borderRadius: BorderRadius.circular(6), child: LinearProgressIndicator(value: p.progress.clamp(0.0, 1.0), minHeight: phone ? 7 : 6))),
        const SizedBox(width: 10),
        Text('$progressPct %', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w900, color: scheme.primary)),
      ]),
      const SizedBox(height: 10),
      Wrap(
        spacing: phone ? 10 : 14,
        runSpacing: phone ? 9 : 8,
        children: [
          _projectStageMini(c, 'Lecture', p.reading, Icons.menu_book_outlined),
          _projectStageMini(c, 'Mains ensemble', p.handsTogether, Icons.pan_tool_alt_outlined),
          _projectStageMini(c, 'Mémorisation', p.memory, Icons.psychology_outlined),
          _projectStageMini(c, 'Interprétation', p.interpretation, Icons.theater_comedy_outlined),
        ],
      ),
      const SizedBox(height: 11),
      Wrap(spacing: 7, runSpacing: 6, children: [
        _sessionMetaPill(c, Icons.speed_outlined, tempo),
        _sessionMetaPill(c, Icons.flag_outlined, target),
        _sessionMetaPill(c, Icons.track_changes_outlined, p.workFocus),
      ]),
      const SizedBox(height: 7),
      Text('Dernière mise à jour : ${_formatProjectDateTime(p.lastModified)}', style: TextStyle(fontSize: 10.5, color: scheme.onSurfaceVariant)),
    ]),
  );
}
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (measuredCompletedRuns.length > 1) ...[
                  ...
                  ...measuredCompletedRuns.take(4).map((s) {
                    ...
                    final targetPercent = project.targetTempo > 0 && (s.runThroughEndTempo ?? 0) > 0
                        ? ((s.runThroughEndTempo! / project.targetTempo).clamp(0.0, 1.0) * 100).round()
                        : null;
                    final statusLabel = s.runThroughCompleted == true ? '✅' : '⏹️';
                    return Padding(
                      padding: const EdgeInsets.symmetric(vertical: 3),
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        SizedBox(width: 42, child: Text(dateLabel, style: TextStyle(fontSize: 9.5, color: scheme.onSurfaceVariant))),
                        Expanded(child: Text('$statusLabel ${s.runThroughStartTempo ?? '—'} → ${s.runThroughEndTempo ?? '—'} BPM${targetPercent != null ? ' · $targetPercent % cible' : ''}', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700))),
                        if (delta != null)
              ...
              Container(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    const SizedBox(height: 2),
                    Text('$runMastery% de maîtrise · meilleur run-through ${bestRunTempo} BPM', style: TextStyle(fontSize: 10.5, color: scheme.onSurfaceVariant)),
                  ])),
...
class _ProjectDialogState extends State<ProjectDialog> {
  ...
  Widget build(BuildContext c) {
    ...
    return Theme(data: flatTheme, child: AlertDialog(
      ...
      actionsPadding: EdgeInsets.fromLTRB(phone ? 12 : 18, phone ? 8 : 4, phone ? 12 : 18, phone ? 12 : 14),
      actionsOverflowDirection: VerticalDirection.down,
      actionsOverflowButtonSpacing: 8,
      title: phone && editing && widget.existing != null
          ? Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
              Text(widget.existing!.emoji, style: const TextStyle(fontSize: 25)),
              const SizedBox(width: 9),
              Expanded(child: Text('Modifier le morceau', style: TextStyle(fontSize: phone ? 18 : 20, fontWeight: FontWeight.w800))),
            ])
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          ...
          mainAxisSize: MainAxisSize.min,
          children: [
            if (phone && widget.existing != null) ...[
              _projectEditSummary(c, widget.existing!),
              _dialogSectionGap(c, phone: 12),
            ],
            ...
            Container(
              width: double.infinity,
              padding: EdgeInsets.fromLTRB(phone ? 8 : 10, phone ? 8 : 9, phone ? 8 : 10, phone ? 2 : 3),
              decoration: BoxDecoration(
                color: Theme.of(c).colorScheme.surfaceContainerLow.withOpacity(phone ? .76 : .55),
                borderRadius: BorderRadius.circular(phone ? 16 : 13),
                border: Border.all(color: Theme.of(c).colorScheme.outlineVariant.withOpacity(.68)),
              ),
              child: Column(children: [
                _progressSlider('📖  Lecture / mains séparées', reading, (v) => setState(() => reading = v)),
                _progressSlider('🤝  Mains ensemble', handsTogether, (v) => setState(() => handsTogether = v)),
            ...
            if (widget.existing != null) ...[
              ...
              if (widget.projectSessions.isEmpty)
                ...
                ...widget.projectSessions.map((s) {
                  final scheme = Theme.of(c).colorScheme;
                  final phoneSession = _isPhoneLayout(c);
                  final feelingLabel = s.coachFeeling == 'facile' ? '😊 Facile' : s.coachFeeling == 'correct' ? '😐 Correct' : s.coachFeeling == 'difficile' ? '😓 Difficile' : '';
                  final feelingColor = s.coachFeeling == 'difficile' ? scheme.error : s.coachFeeling == 'facile' ? scheme.primary : scheme.onSurfaceVariant;
                  return Padding(
                    padding: const EdgeInsets.only(bottom: 8),
                    child: Material(
                      color: Colors.transparent,
                      child: InkWell(
                        borderRadius: BorderRadius.circular(phoneSession ? 14 : 11),
                        onTap: () => _showProjectSessionDetails(c, s),
                        child: Container(
                          padding: EdgeInsets.fromLTRB(phoneSession ? 11 : 10, phoneSession ? 11 : 9, phoneSession ? 9 : 10, phoneSession ? 11 : 9),
                          decoration: BoxDecoration(
                            color: scheme.surfaceContainerHighest.withOpacity(phoneSession ? .50 : .35),
                            borderRadius: BorderRadius.circular(phoneSession ? 14 : 11),
                            border: Border.all(color: scheme.outlineVariant.withOpacity(.62)),
                          ),
                          child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                            Container(
                              width: phoneSession ? 38 : 34,
                              height: phoneSession ? 38 : 34,
                              alignment: Alignment.center,
                              decoration: BoxDecoration(color: scheme.primaryContainer.withOpacity(.62), borderRadius: BorderRadius.circular(10)),
                              child: Icon(s.type == 'Run-through' ? Icons.play_circle_outline : Icons.music_note_outlined, size: phoneSession ? 19 : 17, color: scheme.primary),
                            ),
                            const SizedBox(width: 10),
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(children: [
                                Expanded(child: Text(s.plannedDuration != null ? '${s.plannedDuration} → ${s.duration} min · ${s.type}' : '${s.duration} min · ${s.type}', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: phoneSession ? 12.5 : 12, fontWeight: FontWeight.w800))),
                                if (s.rating > 0) Text('★' * s.rating, style: const TextStyle(color: Colors.amber, fontSize: 10.5)),
                              ]),
                              const SizedBox(height: 3),
                              Wrap(spacing: 7, runSpacing: 3, children: [
                                Text('${s.date.day.toString().padLeft(2, '0')}/${s.date.month.toString().padLeft(2, '0')}/${s.date.year}', style: TextStyle(fontSize: 10.5, color: scheme.onSurfaceVariant)),
                                if (feelingLabel.isNotEmpty) Text(feelingLabel, style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: feelingColor)),
                                if (s.newTempo != null) Text('${s.newTempo} BPM', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: scheme.primary)),
                                if (s.type == 'Run-through' && s.runThroughCompleted != null)
                                  Text(s.runThroughCompleted! ? '✅ Terminé' : '⏹️ Interrompu', style: TextStyle(fontSize: 10.5, fontWeight: FontWeight.w700, color: s.runThroughCompleted! ? scheme.primary : scheme.error)),
                              ]),
                              if (s.coachFocus != null && s.coachFocus!.trim().isNotEmpty) ...[
                                const SizedBox(height: 3),
                                Text('Focus · ${s.coachFocus}', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 10, color: scheme.onSurfaceVariant)),
                              ],
                            ])),
                            const SizedBox(width: 6),
...
class _WeekState extends State<Week> with SingleTickerProviderStateMixin {
  ...
  Widget _nextSessionCoachSummary(BuildContext c, PlanItem? item) {
    if (item == null) return const SizedBox.shrink();
    final scheme = Theme.of(c).colorScheme;
    final today = _isToday(item.date);
    final p = widget.projectById(item.projectId);
    final focus = item.coachNewFocus ?? p?.workFocus;
    final reason = (item.coachAdjustmentReason ?? '').trim();
    final dateLabel = today
        ? "Aujourd'hui"
        : '${weekday(item.date.weekday)} ${item.date.day}/${item.date.month}';
    return Container(
      width: double.infinity,
      margin: EdgeInsets.only(bottom: phoneSafe(c) ? 12 : 14),
      padding: EdgeInsets.fromLTRB(phoneSafe(c) ? 13 : 15, phoneSafe(c) ? 12 : 13, phoneSafe(c) ? 11 : 15, phoneSafe(c) ? 12 : 13),
      decoration: BoxDecoration(
        color: scheme.primary.withOpacity(.055),
        borderRadius: BorderRadius.circular(14),
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.play_arrow_rounded, size: 14, color: scheme.primary),
                  const SizedBox(width: 4),
                  Text('PROCHAINE SÉANCE', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w900, letterSpacing: .45, color: scheme.primary)),
                ]),
              ),
              const Spacer(),
              Text(dateLabel, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: scheme.onSurfaceVariant)),
            ],
          ),
          const SizedBox(height: 9),
          ...
          Wrap(spacing: 6, runSpacing: 5, children: [
            ...
            if (focus != null && focus.trim().isNotEmpty) _phoneMetaPill(c, Icons.gps_fixed_outlined, focus),
            if (item.method != null && item.method!.trim().isNotEmpty) _phoneMetaPill(c, Icons.menu_book_outlined, item.method!, muted: true),
          ]),
          if (reason.isNotEmpty) ...[
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              const SizedBox(width: 6),
              Expanded(child: Text(reason, style: TextStyle(fontSize: 10.5, height: 1.25, color: scheme.onSurfaceVariant))),
            ]),
          ],
        ],
      ),
  ...
  bool phoneSafe(BuildContext c) => MediaQuery.sizeOf(c).width < 600;
  ...
  Widget build(BuildContext c) {
    ...
    final pending = [...widget.items]..removeWhere((x) => x.completed);
    pending.sort((a, b) => a.date.compareTo(b.date));
    final pendingToday = pending.where((x) => _isToday(x.date)).toList();
    final pendingFuture = pending.where((x) {
      final d = DateTime(x.date.year, x.date.month, x.date.day);
      return d.isAfter(todayKey);
    }).toList();
    final pendingOverdue = pending.where((x) {
      final d = DateTime(x.date.year, x.date.month, x.date.day);
      ...
      children: [
          ...
          actions: [
            if (!phone) ...[
              ...
              PopupMenuButton<String>(
                ...
                onSelected: (value) {
                  switch (value) {
                    case 'propose': widget.onPropose(); break;
                    case 'coach': widget.onOpenCoachLog(); break;
                    case 'capacity': widget.onEditCapacity(); break;
                    case 'routine': widget.onOpenRoutine(); break;
                    case 'clear': widget.onClear(); break;
                  }
                },
                itemBuilder: (_) => const [
                  PopupMenuItem(value: 'propose', child: Text('✨  Proposer un planning')),
                  PopupMenuItem(value: 'coach', child: Text('🧠  Journal du coach')),
                  PopupMenuItem(value: 'capacity', child: Text('⚙️  Disponibilités')),
                  PopupMenuItem(value: 'routine', child: Text('🔁  Ma routine')),
                  PopupMenuDivider(),
                  PopupMenuItem(value: 'clear', child: Text('🗑️  Effacer le planning')),
                ],
              ),
            ],
          ],
        ...
        Expanded(
          child: ListView(
            ...
            padding: EdgeInsets.fromLTRB(compactPhone ? 12 : 16, 12, compactPhone ? 12 : 16, 24),
            children: [
              CardBox(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                      ...
                      SingleChildScrollView(
                        ...
                        child: Row(
                          children: cats.map((cat) {
                            final plannedMin = widget.planned[cat] ?? 0;
                            final recommendedMin = widget.recommended[cat] ?? 0;
                            return Padding(
                              padding: const EdgeInsets.only(right: 7),
                              child: Container(
                                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 7),
                                decoration: BoxDecoration(
                                  color: categoryColor(cat).withOpacity(.09),
                                  borderRadius: BorderRadius.circular(12),
                                ),
                                child: Row(mainAxisSize: MainAxisSize.min, children: [
                                  Icon(categoryIcon(cat), size: 14, color: categoryColor(cat)),
                                  const SizedBox(width: 5),
                                  Text(cat, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700)),
                                  const SizedBox(width: 5),
                                  Text('$plannedMin/$recommendedMin min', style: TextStyle(fontSize: 10.5, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                                ]),
                              ),
                            );
                          }).toList(),
                    ...
                    const SizedBox(height: 4),
                    if (phone && nextPendingId != null)
                      _nextSessionCoachSummary(c, widget.items.where((x) => x.id == nextPendingId).firstOrNull),
                    Align(
                      alignment: Alignment.centerRight,
                      child: TextButton.icon(
                  ...
                  return displayEntries.expand((entry) => [
                      ...
                      ...entry.value.map((x) => Padding(
                            ...
                            child: swipeToDelete(
                              ...
                              child: CardBox(
                                accentColor: phone ? (x.completed ? Colors.green.shade600 : categoryColor(x.category)) : null,
                                child: Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Container(
...
class _BilanScreenState extends State<BilanScreen> {
  ...
  Widget _planComparisonRow(BuildContext c, WeeklyPlanComparison item) {
    ...
    final statusText = item.completed
        ? '${item.realizedMinutes} min réalisé · ${((item.adherence ?? 0) * 100).round()}%'
  ...
  Widget _buildCurrentWeek(BuildContext c) {
    ...
    return ListView(
      ...
      children: [
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Expanded(child: _statTile(
                ...
                label: 'Prévu à ce stade',
                value: '${bilan.duePlannedMinutes} min',
                sub: '${bilan.duePlannedSessions} séance${bilan.duePlannedSessions > 1 ? 's' : ''} échue${bilan.duePlannedSessions > 1 ? 's' : ''}',
              )),
              const SizedBox(width: 10),
              Expanded(child: _statTile(
                c,
                ...
                label: 'Réalisé',
                value: '${bilan.realizedPlannedMinutes} min',
                sub: '${bilan.completedPlannedSessions} séance${bilan.completedPlannedSessions > 1 ? 's' : ''}',
              )),
            ]),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: _statTile(
                ...
                label: 'À venir',
                value: '${bilan.upcomingPlannedMinutes} min',
                sub: '${bilan.upcomingPlannedSessions} séance${bilan.upcomingPlannedSessions > 1 ? 's' : ''}',
              )),
              const SizedBox(width: 10),
              Expanded(child: _statTile(
                c,
                icon: Icons.calendar_today_outlined,
                label: 'Planifié semaine',
                value: '${bilan.plannedMinutes} min',
                sub: '${bilan.plannedSessions} séance${bilan.plannedSessions > 1 ? 's' : ''}',
              )),
            ]),
            if (bilan.duePlannedMinutes > 0) ...[
              ...
              Builder(builder: (_) {
                ...
                return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Expanded(child: Text('Adhérence des séances échues : $pct %', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: color))),
                  ...
                  const SizedBox(height: 4),
                  Text(
                    'Le pourcentage compare uniquement les séances dont la date est passée ou celle d’aujourd’hui. Les séances à venir restent hors du calcul.',
                    style: TextStyle(fontSize: 10.5, color: Theme.of(c).colorScheme.onSurfaceVariant),
                  ),
                ]);
              }),
              ...
              const SizedBox(height: 10),
              Text(
                'Aucune séance planifiée n’est échue pour le moment : le taux prévu → réalisé apparaîtra après la première séance due.',
                style: TextStyle(fontSize: 10.5, color: Theme.of(c).colorScheme.onSurfaceVariant),
              ),
            ],
          ]),
        ...
        const SizedBox(height: 16),
        if (bilan.planComparisons.isNotEmpty)
          CardBox(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                ...
                for (final item in bilan.planComparisons)
                  Padding(
                    padding: const EdgeInsets.only(bottom: 12),
                    child: _planComparisonRow(c, item),
                  ),
              ],
            ),
          ),
        if (bilan.planComparisons.isNotEmpty) const SizedBox(height: 4),
    ...
    Widget metric(String label, double? value, String valueText) {
      ...
      return Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: accent.withOpacity(compact ? .22 : .18)),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(children: [
              Icon(icon, size: compact ? 15 : 13, color: accent),
              const SizedBox(width: 5),
              Expanded(child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: compact ? 9.5 : 9.2, height: 1.1, fontWeight: FontWeight.w900, color: scheme.onSurfaceVariant, letterSpacing: .25))),
            ]),
            const SizedBox(height: 5),
            Text(valueText, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: compact ? 14.5 : 14, height: 1.08, fontWeight: FontWeight.w900, color: scheme.onSurface)),
            if (value != null) ...[
              const SizedBox(height: 6),
              ClipRRect(
                borderRadius: BorderRadius.circular(6),
                child: LinearProgressIndicator(value: value.clamp(0.0, 1.0).toDouble(), minHeight: compact ? 5 : 5),
              ),
            ],
          ],
        ),
        ...
        children: [
          ...
          CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Builder(builder: (context) {
              ...
              Widget _masteryBadge(BuildContext context, IconData icon, String label, {bool primary = false, bool warning = false, bool success = false}) {
                ...
                final bg = warning
                    ...
                    : success
                        ...
                        : primary
                            ? scheme.primaryContainer.withOpacity(.52)
                            : scheme.surfaceContainerHighest.withOpacity(.70);
                return Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(
                    color: bg,
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(color: base.withOpacity(.18)),
                  ),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(icon, size: 13, color: base),
                    const SizedBox(width: 4),
                    Text(label, style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w800, color: base)),
                  ]),
                );
              }
              ...
              // V237 — Maîtrise des morceaux : refonte esthétique iPhone, score mieux mis en valeur, badges et métriques plus visuels
// V236 : les cartes de maîtrise sont rendues indépendantes visuellement ;
              // on conserve ici une seule liste source afin d'éviter tout dédoublement de données.
                        ...
                        child: Material(
                          ...
                          child: InkWell(
                            ...
                            child: Container(
                              ...
                              padding: EdgeInsets.fromLTRB(compact ? 12 : 12, compact ? 12 : 11, compact ? 12 : 12, compact ? 13 : 12),
                              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
                                Container(
                                  width: compact ? 40 : 34,
                                  ...
                                  alignment: Alignment.center,
                                  decoration: BoxDecoration(
                                    color: masteryAccent.withOpacity(.12),
                                    shape: BoxShape.circle,
                                    border: Border.all(color: masteryAccent.withOpacity(.28)),
                                  ),
                                  child: Text(p.emoji, style: TextStyle(fontSize: compact ? 20 : 18)),
                                ),
                                const SizedBox(width: 9),
                                Expanded(
                                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    Text(p.name, maxLines: compact ? 2 : 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: compact ? 15 : 14.5, fontWeight: FontWeight.w900, height: 1.15)),
                                    const SizedBox(height: 3),
                                    Text(masteryLabel(score), style: TextStyle(fontSize: compact ? 10.5 : 9.5, fontWeight: FontWeight.w700, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                                  ]),
                                ),
                                const SizedBox(width: 7),
                                Container(
                                  padding: EdgeInsets.symmetric(horizontal: compact ? 10 : 8, vertical: compact ? 7 : 6),
                                  decoration: BoxDecoration(
                                    color: Theme.of(c).colorScheme.primaryContainer.withOpacity(.62),
                                    borderRadius: BorderRadius.circular(14),
                                    border: Border.all(color: Theme.of(c).colorScheme.primary.withOpacity(.18)),
                                  ),
                                  child: Text('${score.round()} %', style: TextStyle(fontSize: compact ? 16 : 14, fontWeight: FontWeight.w900, color: Theme.of(c).colorScheme.primary)),
                                ),
                                const SizedBox(width: 2),
                                Icon(Icons.chevron_right, size: 18, color: Theme.of(c).colorScheme.onSurfaceVariant),
                              ]),
                              const SizedBox(height: 9),
                              ClipRRect(
                                borderRadius: BorderRadius.circular(99),
                                child: LinearProgressIndicator(value: (score / 100).clamp(0.0, 1.0), minHeight: compact ? 8 : 7),
                              ),
                              if (compact) ...[
                                const SizedBox(height: 6),
                                Wrap(
                                  ...
                                  runSpacing: 6,
                                  children: [
                                    if (p.priority) _masteryBadge(c, Icons.star_outline, 'Priorité', primary: true),
                                    if (p.effectiveStatus == 'Répertoire d’entretien') _masteryBadge(c, Icons.build_circle_outlined, 'Entretien'),
                              ...
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  borderRadius: BorderRadius.circular(compact ? 13 : 11),
                                  color: Theme.of(c).colorScheme.primaryContainer.withOpacity(compact ? .46 : .42),
                                  border: Border.all(color: Theme.of(c).colorScheme.primary.withOpacity(.18)),
                                ),
                                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                  ...
                                  const SizedBox(width: 8),
                                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    Text('POINT À TRAVAILLER', style: TextStyle(fontSize: 9.5, fontWeight: FontWeight.w900, letterSpacing: .45, color: Theme.of(c).colorScheme.primary)),
                                    const SizedBox(height: 2),
                                    Text(
                                      '$limiting${latestRun != null ? ' · Dernier Run-through : ${_formatProjectDateTime(latestRun.date)}' : ''}',
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Container(
              padding: EdgeInsets.symmetric(horizontal: phone ? 12 : 10, vertical: phone ? 4 : 2),
              decoration: BoxDecoration(
                color: Theme.of(c).colorScheme.surfaceContainer,
                borderRadius: BorderRadius.circular(phone ? 13 : 10),
                border: Border.all(color: Theme.of(c).colorScheme.outlineVariant.withOpacity(phone ? .64 : .42)),
              ),
              child: Row(children: [
                const Expanded(child: Text('Fréquence', style: TextStyle(fontWeight: FontWeight.w700))),
                IconButton(
                  tooltip: 'Diminuer',
                  visualDensity: VisualDensity.compact,
                  icon: const Icon(Icons.remove_circle_outline),
                ...
                SizedBox(width: 44, child: Center(child: Text('$count×', style: const TextStyle(fontWeight: FontWeight.w900)))),
                IconButton(
                  tooltip: 'Augmenter',
                  visualDensity: VisualDensity.compact,
                  icon: const Icon(Icons.add_circle_outline),
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            InkWell(
              ...
              child: Container(
                padding: EdgeInsets.symmetric(horizontal: phone ? 13 : 10, vertical: phone ? 10 : 7),
                decoration: BoxDecoration(
                  color: Theme.of(c).colorScheme.surfaceContainer,
                  borderRadius: BorderRadius.circular(phone ? 13 : 10),
                  border: Border.all(color: Theme.of(c).colorScheme.outlineVariant.withOpacity(phone ? .68 : .45)),
                ),
                child: Row(
                  children: [
                    Icon(Icons.calendar_month_outlined, size: 20, color: Theme.of(c).colorScheme.primary),
                    const SizedBox(width: 10),
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      const Text('Date de la séance', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700)),
                      const SizedBox(height: 2),
                      Text('${date.day.toString().padLeft(2, '0')}/${date.month.toString().padLeft(2, '0')}/${date.year}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w800)),
                    ])),
