...
// V185 — Hiérarchie des écrans de détail : sections iPhone plus lisibles avec séparateurs légers, sans modifier le fonctionnel.
...
InputDecorationThemeData _mobileFriendlyDialogInputs(BuildContext context) {
  ...
  return base.copyWith(
    ...
    focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(13), borderSide: BorderSide(color: scheme.primary, width: 1.4)),
    errorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(13), borderSide: BorderSide(color: scheme.error, width: 1.2)),
    focusedErrorBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(13), borderSide: BorderSide(color: scheme.error, width: 1.4)),
    filled: true,
    fillColor: scheme.surfaceContainerHighest.withOpacity(.48),
    isDense: false,
...
Widget _detailSectionHeader(BuildContext context, String title, {IconData? icon}) {
  ...
  final scheme = Theme.of(context).colorScheme;
  return Padding(
    padding: EdgeInsets.only(top: phone ? 15 : 14, bottom: phone ? 7 : 6),
    child: Column(
      ...
      children: [
        Row(
          children: [
            if (icon != null) ...[
              Icon(icon, size: phone ? 18 : 17, color: scheme.primary),
        ...
        if (phone) ...[
          ...
          Divider(height: 1, thickness: 1, color: scheme.outlineVariant.withOpacity(.42)),
        ],
      ],
    ),
  );
  ...
  Widget build(BuildContext c) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: phone
            ? (accentColor == null ? null : Border(left: BorderSide(color: accentColor!, width: 3)))
              ...
              child: swipeToDelete(
                ...
                child: CardBox(
                  ...
                  child: InkWell(
                    ...
                    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Container(width: phone ? 38 : 42, height: phone ? 38 : 42, alignment: Alignment.center,
                        decoration: BoxDecoration(color: categoryColor(s.type).withOpacity(.14), borderRadius: BorderRadius.circular(12)),
                        child: Icon(categoryIcon(s.type), size: phone ? 18 : 19, color: categoryColor(s.type))),
                      const SizedBox(width: 10),
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(children: [
                          Expanded(child: Text(project?.name ?? 'Pratique libre', maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: phone ? 15 : 15.5, fontWeight: FontWeight.w800, height: 1.15))),
                          if (s.rating > 0) ...[
                            const SizedBox(width: 5),
                            Text('★' * s.rating, style: const TextStyle(color: Colors.amber, fontSize: 12.5)),
                          ],
                        ]),
                        const SizedBox(height: 3),
                        Text('${s.date.day.toString().padLeft(2,'0')}/${s.date.month.toString().padLeft(2,'0')}/${s.date.year} · ${s.duration} min', style: TextStyle(fontSize: 11, color: Theme.of(c).colorScheme.onSurfaceVariant)),
                        const SizedBox(height: 7),
                        Wrap(spacing: 5, runSpacing: 5, children: [
                          ...
                          if (feeling.isNotEmpty) _sessionMetaPill(c, Icons.psychology_outlined, feeling),
                          if (s.plannedDuration != null && s.plannedDuration! > 0) _sessionMetaPill(c, Icons.schedule_outlined, '${s.plannedDuration} → ${s.duration} min'),
                        ]),
...
class _SessionDialogState extends State<SessionDialog> {
  ...
  Widget _sectionHeader(BuildContext c, String title, IconData icon) {
    final phone = _isPhoneLayout(c);
    final scheme = Theme.of(c).colorScheme;
    return Padding(
      padding: EdgeInsets.only(top: phone ? 9 : 8, bottom: phone ? 7 : 6),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Row(
            children: [
              Icon(icon, size: phone ? 18 : 18, color: scheme.primary),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  title,
                  style: TextStyle(fontSize: phone ? 12.5 : 12, fontWeight: FontWeight.w900, letterSpacing: phone ? .45 : .35),
                ),
              ),
            ],
          ),
                            ...
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              ...
                              Row(children: [
                                Container(
                                  ...
                                  child: Text(projectStatusEmoji(p.effectiveStatus), style: TextStyle(color: statusColor, fontSize: 12)),
                                ),
                                const SizedBox(width: 7),
