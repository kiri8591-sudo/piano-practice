...
class _PianoPracticeAppState extends State<PianoPracticeApp> {
  ...
  bool _adaptRemainingWeekToReality() {
    ...
    final notDoneEvidence = notDoneMinutes >= 20;
      ...
      switch (weeklyRealitySignal) {
        case 'under':
          switch (weeklyRealityCause) {
            ...
            case 'not_done':
              return 'la semaine est en retrait avec des séances passées encore non réalisées';
            case 'removed':
...
class _WeekState extends State<Week> with SingleTickerProviderStateMixin {
  ...
  Color? _planLifecycleColor(BuildContext c, PlanItem item) {
    if (_planLifecycleLabel(item) == 'Non réalisée') {
