        ...
        ...coachPieces.map((item) => Padding(
          ...
          child: CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              ...
              TextButton(onPressed: () => onStart(PlanItem(id: newId(), date: DateTime.now(), duration: _recommendedMinutesForHome(item.project), title: item.project.name, details: item.action, projectId: item.project.id, category: _focusFor(item.project), method: item.project.method)), child: Text('${_recommendedMinutesForHome(item.project)} min')),
            ]),
          ])),
        )),
      ],
      ...
      if (latestCoachDecision != null) ...[
        ...
        CardBox(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 7),
            const Expanded(child: Text('Dernière analyse du coach', style: TextStyle(fontWeight: FontWeight.w800))),
            Text(_decisionDateLabel(latestCoachDecision!), style: TextStyle(fontSize: 10.5, color: Theme.of(c).colorScheme.onSurfaceVariant)),
          ]),
          const SizedBox(height: 8),
          Text(latestCoachDecision!.message, style: const TextStyle(fontSize: 13, height: 1.3)),
          if ((latestCoachDecision!.reason ?? '').trim().isNotEmpty) ...[
            const SizedBox(height: 5),
            Text('Pourquoi : ${latestCoachDecision!.reason}', style: TextStyle(fontSize: 11.5, color: Theme.of(c).colorScheme.onSurfaceVariant, height: 1.3)),
          ],
          const SizedBox(height: 5),
          Align(alignment: Alignment.centerRight, child: TextButton.icon(onPressed: onOpenCoachLog, icon: const Icon(Icons.history, size: 17), label: const Text('Voir le journal du coach'))),
        ])),
      ],
